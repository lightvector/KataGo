...
bool Search::getPlaySelectionValues(
  ...
  if(&node == rootNode && numChildren > 0) {
    ...
    const SearchNode* bestChild = children[mostVisitedIdx].getIfAllocated();
    ...
    for(int i = 0; i<numChildren; i++) {
      ...
      if(i != mostVisitedIdx)
        playSelectionValues[i] = getReducedPlaySelectionVisits(node, policyProbs, child, totalChildVisits, bestChildExploreSelectionValue);
    }
  }
...
bool Search::getNodeValues(const SearchNode& node, ReportedSearchValues& values) const {
  ...
  double leadSum = node.stats.leadSum.load(std::memory_order_acquire);
  double weightSum = node.stats.weightSum.load(std::memory_order_acquire);
  if(weightSum <= 0.0)
    return false;
...
void Search::appendPVForMove(vector<Loc>& buf, vector<Loc>& scratchLocs, vector<double>& scratchValues, const SearchNode* node, Loc move, int maxDepth) const {
  if(node == NULL)
    return;

  for(int depth = 0; depth < maxDepth; depth++) {
    bool success = getPlaySelectionValues(*node, scratchLocs, scratchValues, 1.0, false);
    if(!success)
      return;
...
AnalysisData Search::getAnalysisDataOfSingleChild(
  ...
  if(child != NULL) {
    ...
    winLossValueSum = child->stats.winLossValueSum.load(std::memory_order_acquire);
    noResultValueSum = child->stats.noResultValueSum.load(std::memory_order_acquire);
    scoreMeanSum = child->stats.scoreMeanSum.load(std::memory_order_acquire);
    scoreMeanSqSum = child->stats.scoreMeanSqSum.load(std::memory_order_acquire);
    leadSum = child->stats.leadSum.load(std::memory_order_acquire);
    utilitySum = child->stats.utilitySum.load(std::memory_order_acquire);
  ...
  if(numVisits <= 0 || weightSum <= 1e-30) {
...
void Search::getAnalysisData(
  ...
  float policyProbs[NNPos::MAX_NN_POLICY_SIZE];
  {
    ...
    bool success = getPlaySelectionValues(node, scratchLocs, scratchValues, 1.0, false, alwaysComputeLcb, lcbBuf, radiusBuf);
    if(!success)
      return;
...
double Search::getAverageTreeOwnershipHelper(vector<double>& accum, int64_t minVisits, double desiredWeight, const SearchNode* node) const {
  ...
  int numChildren = visitsBuf.size();
  ...
  for(int i = 0; i<numChildren; i++) {
    ...
    actualWeightFromChildren += getAverageTreeOwnershipHelper(accum,minVisits,desiredWeightFromChild,child);
  }

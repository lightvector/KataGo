...
static void runAndUploadSingleGame(GameTask gameTask, Logger& logger, const string& seed, ForkData* forkData, string sgfsDir, Rand& rand) {
  ...
  if(gameData != NULL) {
    ...
    WriteSgf::writeSgf(out,gameData->bName,gameData->wName,gameData->endHist,gameData);
    out.close();
...
int MainCmds::contribute(int argc, const char* const* argv) {
  ...
  try {
    TCLAP::CmdLine cmd("Run KataGo to generate training data for distributed training", ' ', Version::getKataGoVersionForHelp(),true);

 ...
 */
R"%%(
// A is pre-padded to shape [C, hwSizePadded] (per batch element), where hwSizePadded is a
// multiple of MWG. This ensures WMMA .global loads have properly aligned stride.
// B is shape [C, OC], row major.
// Output C is shape [OC, hwSize] (per batch element) - NOT padded, uses hwSize stride.
...
INLINE_FUNC void hGemmWmmaCHWBody(
  const int cSize, const int hwSize, const int hwSizePadded, const int ocSize,
  ...
  for(int kwg = 0; kwg < cSize; kwg += KWG) {
    ...
    for(int kWaveOffset = 0; kWaveOffset<KWG; kWaveOffset += KDIM_WMMA) {
      ...
      // Load A from pre-padded global buffer. Stride = hwSizePadded (aligned).
      #pragma promote_to_registers
      int a[MWI][8];
      for(int aWaveId = 0; aWaveId<MWI; aWaveId++) {
        const int aOffset = aWaveId * MWAVE + wmmaBlockId0 * MWARP;
        const int aStride = hwSizePadded;
...
void hgemmWmmaNCHW(
  ...
  const int a_offset = batch * cSize * hwSizePadded;
  ...
  #if SB == 1
    hGemmWmmaCHWBody(cSize, hwSize, hwSizePadded, ocSize, agm_, bgm_, cgm_, blm, clm);
  #else
    hGemmWmmaCHWBody(cSize, hwSize, hwSizePadded, ocSize, agm_, bgm_, cgm_, clm);
  #endif
}

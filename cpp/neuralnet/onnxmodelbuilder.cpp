...
Result build(
  ...
  // kernel input ports in a different order. Declaring InputMask first makes the two
  // disagree, and the EP misroutes the [N,1,H,W] mask tensor into the InputSpatial port,
  ...
  // (observed on ORT 1.29 + OpenVINO 2026.2, Intel Arc B580). Declaring InputSpatial,
  // InputGlobal, then InputMask fixes it. This is an empirical requirement of that EP
  // rather than the graph's own first-reference order, in which InputMask comes first
  // because the !requireExactNNLen branch below references it. InputMeta, when the model
  // has a metadata encoder, goes after InputGlobal and before InputMask.

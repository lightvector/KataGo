...
struct Buffers {
  ...
  Buffers(CudaHandles* cudaHandles, const Model& m, const ScratchBuffers& scratch) {
    ...
    CUDA_ERR("Buffers",cudaMalloc((void**)&inputBufFloat, inputBufBytesFloat));
    CUDA_ERR("Buffers",cudaMalloc(&inputBuf, inputBufBytes));
    CUDA_ERR("Buffers",cudaMalloc((void**)&inputGlobalBufFloat, inputGlobalBufBytesFloat));
    ...
    CUDA_ERR("Buffers",cudaMalloc((void**)&policyPassBuf, policyPassBufBytes));
    policyBufBytes = m.policyHead->p2Channels * batchXYFloatBytes;
    CUDA_ERR("Buffers",cudaMalloc((void**)&policyBuf, policyBufBytes));

    valueBufBytes = m.valueHead->valueChannels * batchFloatBytes;
    CUDA_ERR("Buffers",cudaMalloc((void**)&valueBuf, valueBufBytes));

    scoreValueBufBytes = m.valueHead->scoreValueChannels * batchFloatBytes;
    CUDA_ERR("Buffers",cudaMalloc((void**)&scoreValueBuf, scoreValueBufBytes));
...
bool NeuralNet::testEvaluateGlobalPoolingResidualBlock(
  ...
  CudaUtils::mallocAndCopyToDevice("deviceInput", inputBuffer.data(), numInputFloats, deviceInput, useFP16);
  CudaUtils::mallocAndCopyToDevice("deviceMask", maskBuffer.data(), numMaskFloats, deviceMask, useFP16);
  CUDA_ERR("deviceMaskFloat",cudaMalloc((void**)&deviceMaskFloat, numMaskFloats * sizeof(float)));
  CUDA_ERR("deviceMaskSum",cudaMalloc((void**)&deviceMaskSum, numMaskSumFloats * sizeof(float)));

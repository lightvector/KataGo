...
* `neuralnet` - Neural net GPU implementation and interface. Contains OpenCL, CUDA, TensorRT, Metal, Eigen, and ONNX backends along with common interfaces and model data structures.
  ...
  * `{cuda,opencl,eigen,trt,metal,onnx,dummy}backend.cpp` - Various backends.

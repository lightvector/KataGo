...
* `neuralnet` - Neural net GPU implementation and interface. Contains OpenCL, CUDA, Eigen, TensorRT, ROCm, ONNX, Metal, MLX backends along with common interfaces and model data structures.
  ...
  * `{cuda,opencl,eigen,trt,rocm,onnx,metal,mlx,dummy}backend.cpp` - Various backends.

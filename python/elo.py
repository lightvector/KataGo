...
class EloInfo:
    ...
    def __str__(self) -> str:
        lines = [f"{player:20s}: {self.elo[player]:8.2f} +/- {self.elo_stderr[player]:5.2f}" for player in self.players]
        return "\n".join(lines)
...
class Likelihood:
    ...
    def __init__(
        ...
        self.kind = kind
        assert kind in [
            Likelihood.SIGMOID_KIND,
            Likelihood.GAUSSIAN_KIND,
        ], "invalid kind"
...
def likelihood_of_games(
    ...
    if num_games > 0.0:
        if include_first_player_advantage:

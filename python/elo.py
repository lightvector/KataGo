...
class EloInfo:
    ...
    def __init__(
        ...
        self.players = sorted(
            self.players,
            key=(lambda player: -elo[player] if player != P1_ADVANTAGE_NAME else 1e50),
        )
    ...
    def __str__(self) -> str:
        ...
        for player in self.players:
            lines.append(
                f"{player:20s}: {self.elo[player]:8.2f} +/-"
                f" {self.elo_stderr[player]:5.2f}"
            )
        return "\n".join(lines)
...
class Likelihood:
    ...
    def __init__(
        ...
        self.kind = kind
        assert (
            kind == Likelihood.SIGMOID_KIND or kind == Likelihood.GAUSSIAN_KIND
        ), "invalid kind"

    def add_idxs(self, player_to_idx: Dict[Player, PlayerIdx]):
        self.pidxcombo: List[Tuple[PlayerIdx, float]] = [
            (player_to_idx[player], coeff)
            for (player, coeff) in self.playercombo.items()
        ]

    LOG_ONE_OVER_SQRT_TWO_PI = math.log(1.0 / math.sqrt(2.0 * math.pi))

    def get_loglikelihood(self, strengths: np.array) -> float:
        strength_total = self.offset + sum(
            strengths[pidx] * coeff for (pidx, coeff) in self.pidxcombo
        )
        if self.kind == Likelihood.SIGMOID_KIND:
            ...
            return self.weight * (
                Likelihood.LOG_ONE_OVER_SQRT_TWO_PI
                - 0.5 * strength_total * strength_total
            )
    ...
    def accum_dloglikelihood_dstrength(self, strengths: np.array, accum: np.array):
        ...
        if self.kind == Likelihood.SIGMOID_KIND:
            dloglikelihood_dstrength_total = self.weight / (
                1.0 + math.exp(strength_total)
            )
        else:
    ...
    def accum_d2loglikelihood_dstrength2(self, strengths: np.array, accum: np.array):
        ...
        for pidx1, coeff1 in self.pidxcombo:
            for pidx2, coeff2 in self.pidxcombo:
                accum[pidx1, pidx2] += (
                    coeff1 * coeff2 * d2loglikelihood_dstrength_total2
                )
...
def compute_elos(
    ...
    * All players *have* won and lost against other players, but there is nothing that anchors the Elo of the
    ...
    def find_ascent_vector(strengths: np.array) -> np.array:
        ...
        d2loglikelihood_dstrength2 = np.zeros(
    ...
    def line_search_ascend(
        strengths: np.array, cur_loglikelihood: float
    ...
    for i in range(max_iters):
        if verbose:
            print(
                f"Beginning iteration {i}, cur log likelihood {loglikelihood}, last elo"
                f" change {last_elo_change}"
            )

        (new_strengths, new_loglikelihood) = line_search_ascend(
            strengths, loglikelihood
        )
    ...
    for d in data:
        if d.gamecount > 0.0:
            d.accum_d2loglikelihood_dstrength2_scalepow(
                strengths, sqrt_ess_numerator, scale=1.0, power=1.0
            )
            d.accum_d2loglikelihood_dstrength2_scalepow(
                strengths, ess_denominator, scale=1.0 / d.gamecount, power=2.0
            )

    info = EloInfo(
        players=players,
        elo={
            player: ELO_PER_STRENGTH * strengths[player_to_idx[player]]
            for player in players
        },
        elo_stderr={
            player: math.sqrt(
                1.0 / elo_precision[player_to_idx[player], player_to_idx[player]]
            )
            for player in players
        },
        elo_covariance={
            (p1, p2): elo_covariance[player_to_idx[p1], player_to_idx[p2]]
            for p1 in players
            for p2 in players
        },
        effective_game_count={
            player: (
                np.square(
                    sqrt_ess_numerator[player_to_idx[player], player_to_idx[player]]
                )

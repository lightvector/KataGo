...
def load_swa_model_state_dict(state_dict):
    ...
    swa_model_state_dict = {
        key: state_dict["swa_model"][key]
        for key in state_dict["swa_model"]
        if "score_belief_offset_vector" not in key
        and "score_belief_offset_bias_vector" not in key
        and "score_belief_parity_vector" not in key
    }
...
def load_model(checkpoint_file, use_swa, device, pos_len=19, verbose=False):
    ...
    if use_swa:
        if state_dict is None:
            raise ValueError("Cannot use swa without a trained model")
        swa_model_state_dict = load_swa_model_state_dict(state_dict)
        if swa_model_state_dict is None:
            raise ValueError("Checkpoint doesn't contain swa_model")

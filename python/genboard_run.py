...
if __name__ == "__main__":
  ...
  """
    ...
    parser.add_argument(
        "-board", help="Board pattern using {.,*},X,O,? for empty, black, white, unknown", required=True
    )
    parser.add_argument(
        "-turn", help="Approx turn number to tell the net to generate for, [0,300]", required=True, type=float
    )
    parser.add_argument("-turnstdev", help="Approx turn number randomness [0,100]", required=True, type=float)
    parser.add_argument(
        "-source", help="Tell the net to mimic positions from source, {-1,0,1}", required=True, type=int
    )
    parser.add_argument(
        "-verbose",
        help="Print various info and debug messages instead of only the board",
        required=False,
        action="store_true",
    )
    parser.add_argument("-n", help="How many batches to generate, default 1", required=False, type=int, default=1)
    parser.add_argument(
        "-batchsize", help="How many positions to generate, default 1", required=False, type=int, default=1
    )
    ...
    def fail_if_idx_too_large(idx):
        if idx >= size * size:
            raise ValueError("Provided board is larger than 19x19")

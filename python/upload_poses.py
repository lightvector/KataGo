...
if set_total_weight is not None:
    log(f"Setting total weight of data to {str(set_total_weight)}")
    scale = set_total_weight / sumweight
    for pos in poses:
        pos["weight"] *= scale
elif weight_factor is not None:
    log(f"Scaling weight of data by {str(weight_factor)}")
    for pos in poses:
        pos["weight"] *= weight_factor


def postStuff(to_post):
    url = f"{base_server_url}api/startposes/"
...
if connection_config_file is None:
    ...
    for pos in poses:
        weight = pos["weight"]
        data = {
            "run": f"{base_server_url}api/runs/{run_name}/",
            "weight": weight,
            "data": pos,
            "notes": notes,
        }
        ...
        if len(to_post) >= 5000:
            ...
            log(f"Posting {num_posted}/{num_total}")
            postStuff(to_post)
            to_post = []

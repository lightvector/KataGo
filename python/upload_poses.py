...
parser.add_argument(
    "-poses", help="startposes or hintposes file or dir", required=True, action="append"
)
parser.add_argument(
    "-upload-log-file", help="log upload data to this file", required=False
)
parser.add_argument(
    "-connection-config",
...
parser.add_argument(
    "-set-total-weight",
    help="set total weight of poses to this",
    type=float,
    required=False,
)
parser.add_argument(
    "-weight-factor",
    help="multiply weight of poses by this",
    type=float,
    required=False,
)
parser.add_argument(
    "-separate-summaries",
    help="summarize dirs separately",
    action="store_true",
    required=False,
)
parser.add_argument(
    "-notes", help="notes or description label for poses", required=False
)
    ...
    if separate_summaries:
        ...
        if sumweight > 0 and sumweightsq > 0:
            ...
            log(
                "%d %f %f"
                % (
                    len(poses_by_key_this_file.values()),
                    sumweight,
                    (sumweight * sumweight / sumweightsq),
                )
            )
    ...
    if os.path.isdir(poses_file_or_dir):
        for path, dirnames, filenames in os.walk(poses_file_or_dir):
            ...
            for filename in filenames:
                if filename.endswith(".startposes.txt") or filename.endswith(
                    ".hintposes.txt"
                ):

...
if __name__ == "__main__":
  ...
  """
    ...
    parser.add_argument(
        "-initial-checkpoint", help="If no training checkpoint exists, initialize from this checkpoint", required=False
    )
    ...
    parser.add_argument(
        "-samples-per-epoch", help="Number of data samples to consider as one epoch", type=int, required=False
    )
    ...
    parser.add_argument(
        "-gnorm-clip-scale", help="Multiplier on gradient clipping threshold", type=float, required=False
    )
    parser.add_argument(
        "-sub-epochs", help="Reload training data up to this many times per epoch", type=int, default=1, required=False
    )
    parser.add_argument(
        "-swa-period-samples", help="How frequently to average an SWA sample, in samples", type=float, required=False
    )
    parser.add_argument(
        "-swa-scale", help="Number of samples to average in expectation together for SWA", type=float, required=False
    )
    ...
    parser.add_argument(
        "-max-epochs-this-instance", help="Terminate training after this many more epochs", type=int, required=False
    )
    ...
    parser.add_argument(
        "-randomize-val", help="Randomize order of validation files", required=False, action="store_true"
    )
    ...
    parser.add_argument(
        "-brenorm-avg-momentum", type=float, help="Set brenorm running avg rate to this value", required=False
    )
    parser.add_argument(
        "-brenorm-target-rmax", type=float, help="Gradually adjust brenorm rmax to this value", required=False
    )
    parser.add_argument(
        "-brenorm-target-dmax", type=float, help="Gradually adjust brenorm dmax to this value", required=False
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-soft-policy-weight-scale", type=float, default=1.0, help="Soft policy loss coeff", required=False
    )
    parser.add_argument(
        "-value-loss-scale", type=float, default=1.0, help="Additional value loss coeff", required=False
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-intermediate-loss-scale", type=float, help="Loss factor scale for intermediate head", required=False
    )
    parser.add_argument(
        "-intermediate-distill-scale", type=float, help="Distill factor scale for intermediate head", required=False
    )
...
def main(rank: int, world_size: int, args, multi_gpu_device_ids, readpipes, writepipes, barrier):
    ...
    def get_weight_decay(raw_model, lr_scale, warmup_scale, train_state, running_metrics, group_name):
        if raw_model.get_norm_kind() in ["fixup", "fixscale"]:
            if group_name in ["normal", "normal_gamma", "output"]:
                ...
                assert False
        elif raw_model.get_norm_kind() in [
            "bnorm",
            "brenorm",
            "fixbrenorm",
            "fixscaleonenorm",
        ]:
            if group_name in ["normal", "normal_gamma"]:
                adaptive_scale = 1.0
                if "sums" in running_metrics and "norm_normal_batch" in running_metrics["sums"]:
                    norm_normal_batch = (
                        running_metrics["sums"]["norm_normal_batch"] / running_metrics["weights"]["norm_normal_batch"]
                    )
    ...
    def load():
        if not os.path.exists(get_checkpoint_path()):
            ...
            for i in range(NUM_SHORTTERM_CHECKPOINTS_TO_KEEP):
                if os.path.exists(get_checkpoint_prev_path(i)):
                    raise Exception(
                        f"No preexisting checkpoint found, but {get_checkpoint_prev_path(i)} exists, something is wrong"
    ...
    def update_and_return_lr_and_wd():
        ...
        warmup_scale = 1.0
        if model_config["norm_kind"] in ["fixup", "fixscale", "fixscaleonenorm"]:
        ...
        elif model_config["norm_kind"] in ["bnorm", "brenorm", "fixbrenorm"]:
    ...
    def maybe_update_brenorm_params():
        ...
        if model_config["norm_kind"] in ["brenorm", "fixbrenorm"]:
        ...
        while True:
            ...
            if curdatadir != last_curdatadir:
                if not os.path.exists(curdatadir):
                    logging.info(
                        "Shuffled data path does not exist, there seems to be no shuffled data yet, waiting and trying"
                        " again later: %s" % curdatadir
                    )
                    time.sleep(30)
                    continue
                ...
                if not os.path.exists(trainjsonpath):
                    logging.info(
                        "Shuffled data train.json file does not exist, there seems to be no shuffled data yet, waiting"
                        " and trying again later: %s" % trainjsonpath
                    )
                    time.sleep(30)
                    continue
                ...
                train_files = [
                    os.path.join(tdatadir, fname) for fname in os.listdir(tdatadir) if fname.endswith(".npz")
                ]
            ...
            # Same directory as before, no new shuffle
                ...
                if (
                    max_train_steps_since_last_reload is not None
                    and train_state["train_steps_since_last_reload"] + 0.99 * samples_per_epoch / sub_epochs
    ...
    def get_files_for_subepoch():
        ...
        for filename in trainfilegenerator:
            ...
            if (
                ...
                and batches_to_use_so_far > 0
                and random.random()
                >= (batches_to_use_so_far + num_batches_this_file - num_batches_per_subepoch) / num_batches_this_file
            ):
                break
    ...
    while True:
        ...
        if rank == 0:
            ...
            if export_prob is not None and random.random() > export_prob:
            ...
            if len(val_files) == 0:
                ...
                with torch.no_grad():
                    ...
                    for batch in data_processing_pytorch.read_npz_training_data(
                        ...
                        metrics = detensorify_metrics(metrics)
                        accumulate_metrics(
                            val_metric_sums, val_metric_weights, metrics, batch_size, decay=1.0, new_weight=1.0
                        )

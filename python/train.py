...
if __name__ == "__main__":
  ...
  """
    ...
    parser.add_argument(
        "-traindir",
        help="Dir to write to for recording training results",
        required=True,
    )
    parser.add_argument(
        "-datadir",
        help="Directory with a train and val subdir of npz data",
        required=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-pos-len",
        help="Spatial length of expected training data",
        type=int,
        required=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-lr-scale",
        help="LR multiplier on the hardcoded schedule",
        type=float,
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-lookahead-k", help="Use lookahead optimizer", type=int, required=False
    )
    parser.add_argument(
        "-lookahead-alpha", help="Use lookahead optimizer", type=float, required=False
    )
    parser.add_argument(
        "-lookahead-print",
    ...
    parser.add_argument(
        "-multi-gpus",
        help="Use multiple gpus, comma-separated device ids",
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-export-prob",
        help="Export model with this probablity",
        type=float,
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-sleep-seconds-per-epoch",
        help="Sleep this long between epochs",
        type=int,
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-no-repeat-files",
        help=(
            "Track what shuffled data was used and do not repeat, even when killed and"
            " resumed"
        ),
        required=False,
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "How many samples to adjust brenorm params all but 1/e of the way to target"
        ),
        required=False,
    )
    ...
    parser.add_argument(
        "-main-loss-scale",
        type=float,
        help="Loss factor scale for main head",
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-intermediate-distill-scale",
        type=float,
        help="Distill factor scale for intermediate head",
        required=False,
    )
...
def multiprocessing_setup(rank: int, world_size: int):
    ...
    logging.info(
        f"Returned from torch.distributed.init_process_group, my rank = {rank},"
...
def main(
    ...
    if rank == 0:
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(
                    os.path.join(traindir, f"train{rank}.log"), mode="a"
                ),
            ...
            if path is not None:
                ...
                for i in reversed(range(NUM_SHORTTERM_CHECKPOINTS_TO_KEEP - 1)):
                    if os.path.exists(get_checkpoint_prev_path(i)):
                        os.replace(
                            get_checkpoint_prev_path(i), get_checkpoint_prev_path(i + 1)
                        )
    ...
    def get_weight_decay(
        ...
        if (
            raw_model.get_norm_kind() == "fixup"
            or raw_model.get_norm_kind() == "fixscale"
        ):
            if (
                group_name == "normal"
            ...
            if group_name == "normal" or group_name == "normal_gamma":
                adaptive_scale = 1.0
                if (
                    "sums" in running_metrics
                    and "norm_normal_batch" in running_metrics["sums"]
                ):
                    norm_normal_batch = (
                    ...
                    adaptive_scale = math.pow(
                        2.0, 2.0 * math.tanh(math.log(ratio + 1e-30) * 1.5)
                    )
                ...
                return (
                    ...
                    * batch_size
                    / 256.0
                    * math.pow(lr_scale * warmup_scale, 0.75)
                    * adaptive_scale
                    * gamma_scale
                )
    ...
    def get_param_groups(raw_model, train_state, running_metrics):
        ...
        num_reg_dict_params = (
            len(reg_dict["normal"])
            + len(reg_dict["normal_gamma"])
            + len(reg_dict["output"])
            + len(reg_dict["noreg"])
            + len(reg_dict["output_noreg"])
        )
        assert (
    ...
    def load():
        if not os.path.exists(get_checkpoint_path()):
            ...
            for i in range(NUM_SHORTTERM_CHECKPOINTS_TO_KEEP):
                if os.path.exists(get_checkpoint_prev_path(i)):
                    raise Exception(
                        "No preexisting checkpoint found, but"
                        f" {get_checkpoint_prev_path(i)} exists, something is wrong"
                        " with the training dir"
                    )
            ...
            if initial_checkpoint is not None:
                if os.path.exists(initial_checkpoint):
                    ...
                    raise Exception(
                        "No preexisting checkpoint found, initial checkpoint provided"
                        " is invalid: {initial_checkpoint}"
                    )
            else:
        ...
        if path_to_load_from is None:
            ...
            if rank == 0 and swa_scale is not None:
                ...
                ema_avg = (
                    lambda avg_param, cur_param, num_averaged: avg_param
                    + new_factor * (cur_param - avg_param)
                )
            ...
            if "train_state" in state_dict:
                ...
                logging.info(
                    "WARNING: Train state not found in state dict, using fresh train"
                    " state"
                )
            ...
            if "modelnorm_normal_baseline" not in train_state:
                logging.info(
                    "Computing modelnorm_normal_baseline since not in train state"
                )
                with torch.no_grad():
                    (
                        modelnorm_normal,
                    ...
                    train_state["modelnorm_normal_baseline"] = modelnorm_normal_baseline
                    logging.info(
                        "Model norm normal baseline computed:"
                        f" {modelnorm_normal_baseline}"
                    )
            ...
            if "metrics" in state_dict:
                ...
                logging.info(
                    "WARNING: Metrics not found in state dict, using fresh metrics"
                )
            ...
            if "running_metrics" in state_dict:
                ...
                logging.info(
                    "WARNING: Running metrics not found in state dict, using fresh"
                    " running metrics"
                )
            ...
            if "optimizer" in state_dict:
                ...
                logging.info(
                    "WARNING: Optimizer not found in state dict, using fresh optimizer"
                )
    ...
    if (
        max_train_bucket_per_new_data is not None
        and "train_bucket_level" not in train_state
    ):
    ...
    if intermediate_distill_scale is not None or intermediate_loss_scale is not None:
        ...
        ), "Model must have intermediate head to use intermediate distill or loss"
    ...
    def update_and_return_lr_and_wd():
        ...
        for param_group in optimizer.param_groups:
            ...
            if lookahead_alpha is not None:
                param_group["lr"] = (
                    per_sample_lr * warmup_scale * group_scale / lookahead_alpha
                )
            else:
    ...
    def maybe_update_brenorm_params():
        ...
        if (
            ...
            num_samples_elapsed = (
                ...
                - last_brenorm_update_samples_this_instance
            )
            factor = math.exp(-num_samples_elapsed / brenorm_adjustment_scale)
            train_state["brenorm_rmax"] = train_state["brenorm_rmax"] + (
                1.0 - factor
            ) * (brenorm_target_rmax - train_state["brenorm_rmax"])
            train_state["brenorm_dmax"] = train_state["brenorm_dmax"] + (
                1.0 - factor
            ) * (brenorm_target_dmax - train_state["brenorm_dmax"])

            raw_model.set_brenorm_params(
                brenorm_avg_momentum,
                train_state["brenorm_rmax"],
                train_state["brenorm_dmax"],
            )
            last_brenorm_update_samples_this_instance = train_state[
                "global_step_samples"
            ]
        ...
        while True:
            ...
            if curdatadir != last_curdatadir:
                if not os.path.exists(curdatadir):
                    logging.info(
                        "Shuffled data path does not exist, there seems to be no"
                        " shuffled data yet, waiting and trying again later: %s"
                        % curdatadir
                    )
                    time.sleep(30)
                    continue
                ...
                if not os.path.exists(trainjsonpath):
                    logging.info(
                        "Shuffled data train.json file does not exist, there seems to"
                        " be no shuffled data yet, waiting and trying again later: %s"
                        % trainjsonpath
                    )
                    time.sleep(30)
                    continue
                ...
                if no_repeat_files:
                    ...
                    train_files = [
                        ...
                        if path not in train_state["data_files_used"]
                    ]
                    new_len = len(train_files)
                    logging.info(
                        f"Dropping {old_len-new_len}/{old_len} files in: {tdatadir} as"
                        " already used"
                    )
                ...
                if len(train_files) <= 0:
                    logging.info(
                        f"No new training files found in: {tdatadir}, waiting 30s and"
                        " trying again"
                    )
                    time.sleep(30)
                    continue
                ...
                while len(train_state["old_train_data_dirs"]) > 20:
                    ...
                    train_state["old_train_data_dirs"] = train_state[
                        "old_train_data_dirs"
                    ][1:]
                ...
                def train_files_gen():
                    ...
                    while True:
                        ...
                        for filename in train_files_shuffled:
                            logging.info(
                                "Yielding training file for dataset: " + filename
                            )
            ...
            # Same directory as before, no new shuffle
                ...
                if max_train_steps_since_last_reload is not None:
                    if (
                        ...
                        logging.info(
                            "Too many train steps since last reload, waiting 5m and"
                            " retrying (current %f)"
                            % train_state["train_steps_since_last_reload"]
                        )
                        time.sleep(300)
    ...
    def get_files_for_subepoch():
        ...
        assert (
            rank == 0
        ), "Helper ddp training processes should not call get_files_for_subepoch"
        ...
        for filename in trainfilegenerator:
            ...
            if batches_to_use_so_far + num_batches_this_file > num_batches_per_subepoch:
                ...
                if (
                    ...
                    and random.random()
                    >= (
                        batches_to_use_so_far
                        + num_batches_this_file
                        - num_batches_per_subepoch
                    )
                    / num_batches_this_file
                ):
                    break
    ...
    def accumulate_metrics(
        metric_sums, metric_weights, metrics, batch_size, decay, new_weight
    ):
        if decay != 1.0:
    ...
    def log_metrics(metric_sums, metric_weights, metrics, metrics_out):
        ...
        for metric in metric_sums:
            if metric.endswith("_sum"):
                metrics_to_print[metric[:-4]] = (
                    metric_sums[metric] / metric_weights[metric]
                )
            elif metric.endswith("_batch"):
        ...
        logging.info(
            ", ".join(
                [
                    "%s = %f" % (metric, metrics_to_print[metric])
    ...
    if rank == 0:
        ...
        train_metrics_out = open(
            os.path.join(traindir, f"metrics_train_rank{rank}.json"), "a"
        )
        val_metrics_out = open(
            os.path.join(traindir, f"metrics_val_rank{rank}.json"), "a"
        )
    ...
    while True:
        if rank == 0:
            ...
            if max_train_bucket_per_new_data is not None:
                if train_state["train_bucket_level"] > 0.99 * samples_per_epoch:
                    ...
                    logging.info(
                        "Exceeding train bucket, not enough new data rows, waiting 5m"
                        " and retrying (current level %f)"
                        % train_state["train_bucket_level"]
                    )
                    time.sleep(300)
                    continue
        ...
        logging.info(
            "Currently up to data row " + str(train_state["total_num_data_rows"])
        )
            ...
            for batch in data_processing_pytorch.read_npz_training_data(
                ...
                if use_fp16:
                    with autocast():
                        model_outputs = ddp_model(
                            batch["binaryInputNCHW"], batch["globalInputNC"]
                        )
                ...
                if batch_count_this_epoch % print_train_loss_every_batches == 0:
                    if (
                        ...
                        lower_rclippage = []
                        dclippage = []
                        raw_model.add_brenorm_clippage(
                ...
                if (
                    ...
                    and batch_count_this_epoch % 10 == 0
                ):
                    lr_right_now, normal_weight_decay_right_now = (
                        update_and_return_lr_and_wd()
                    )
                        ...
                        for param_group in optimizer.param_groups:
                            for param in param_group["params"]:
                                ...
                                slow_param_data.add_(
                                    param.data.detach() - slow_param_data,
        ...
        if rank == 0:
            ...
            logging.info(
                "Export cycle counter = " + str(train_state["export_cycle_counter"])
            )
                ...
                with torch.no_grad():
                    ...
                    for batch in data_processing_pytorch.read_npz_training_data(
                        ...
                        if (
                            ...
                            and val_samples > max_val_samples
                        ):
                            break
                        val_metric_sums["nsamp_train"] = running_metrics["sums"][
                            "nsamp"
                        ]
                        val_metric_weights["nsamp_train"] = running_metrics["weights"][
                            "nsamp"
                        ]
                        val_metric_sums["wsum_train"] = running_metrics["sums"]["wsum"]
                        val_metric_weights["wsum_train"] = running_metrics["weights"][
                            "wsum"
                        ]
                    log_metrics(
                        val_metric_sums, val_metric_weights, metrics, val_metrics_out
                    )
                    t1 = time.perf_counter()
        ...
        if rank == 0:
            ...
            if now - last_longterm_checkpoint_save_time >= datetime.timedelta(hours=12):
                ...
                dated_name = datetime.datetime.now(timezone.utc).strftime(
                    "%Y%m%d-%H%M%S"
                )
                save(
                    ddp_model,
...
if __name__ == "__main__":
    ...
    if num_gpus_used > 1:
        ...
        barrier = None
        main(
            rank, world_size, args, multi_gpu_device_ids, readpipes, writepipes, barrier
        )

...
def cross_entropy(pred_logits, target_probs, dim):
    return -torch.sum(
        target_probs * torch.nn.functional.log_softmax(pred_logits, dim=dim), dim=dim
    )
...
def constant_like(data, other_tensor):
    return torch.tensor(
        data, dtype=other_tensor.dtype, device=other_tensor.device, requires_grad=False
    )
...
class Metrics:
    def __init__(self, batch_size: int, world_size: int, raw_model: Model):
        ...
        self.scorebelief_len = 2 * (
            self.pos_len * self.pos_len + EXTRA_SCORE_DISTR_RADIUS
        )

        self.score_belief_offset_vector = (
            raw_model.value_head.score_belief_offset_vector
        )
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]):
        if isinstance(state_dict["moving_unowned_proportion_sum"], torch.Tensor):
            self.moving_unowned_proportion_sum = state_dict[
        ...
        self.moving_unowned_proportion_weight = state_dict[
            "moving_unowned_proportion_weight"
        ]

    def loss_policy_player_samplewise(
        self, pred_logits, target_probs, weight, global_weight
    ):
    ...
    def loss_policy_opponent_samplewise(
        self, pred_logits, target_probs, weight, global_weight
    ):
        assert pred_logits.shape == (self.n, self.policy_len)
        assert target_probs.shape == (self.n, self.policy_len)
    ...
    def loss_ownership_samplewise(
        self, pred_pretanh, target, weight, mask, mask_sum_hw, global_weight
    ):
        ...
        pred_logits = torch.cat((pred_pretanh, -pred_pretanh), dim=1).view(
            self.n, 2, self.pos_area
        )
        target_probs = torch.stack(
            ((1.0 + target) / 2.0, (1.0 - target) / 2.0), dim=1
        ).view(self.n, 2, self.pos_area)
        loss = (
            torch.sum(
                cross_entropy(pred_logits, target_probs, dim=1)
                * mask.view(self.n, self.pos_area),
                dim=1,
            )
            / mask_sum_hw
        )
    ...
    def loss_scoring_samplewise(
        self, pred_scoring, target, weight, mask, mask_sum_hw, global_weight
    ):
        ...
        loss = (
            torch.sum(torch.square(pred_scoring.squeeze(1) - target) * mask, dim=(1, 2))
            / mask_sum_hw
        )
    ...
    def loss_futurepos_samplewise(
        ...
        assert pred_pretanh.shape == (
            self.n,
            self.num_futurepos_values,
            self.pos_len,
            self.pos_len,
        )
        assert target.shape == (
    ...
    def loss_seki_samplewise(
        ...
        unowned_proportion = torch.sum(unowned_target * mask, dim=(1, 2)) / (
            1.0 + mask_sum_hw
        )
        ...
        loss_sign = torch.sum(
            cross_entropy(sign_pred, sign_target, dim=1) * mask, dim=(1, 2)
        )
        ...
        loss_neutral = torch.sum(
            cross_entropy(neutral_pred, neutral_target, dim=1) * mask, dim=(1, 2)
        )
    ...
    def loss_scorebelief_cdf_samplewise(
        self, pred_logits, target_probs, weight, global_weight
    ):
    ...
    def loss_scorebelief_pdf_samplewise(
        self, pred_logits, target_probs, weight, global_weight
    ):
        assert pred_logits.shape == (self.n, self.scorebelief_len)
        assert target_probs.shape == (self.n, self.scorebelief_len)
    ...
    def loss_scorestdev_samplewise(self, pred, scorebelief_logits, global_weight):
        ...
        expected_score_from_belief = torch.sum(
            scorebelief_probs * self.score_belief_offset_vector.view(1, -1),
            dim=1,
            keepdim=True,
        )
        stdev_of_belief = torch.sqrt(
            ...
            + torch.sum(
                scorebelief_probs
                * torch.square(
                    self.score_belief_offset_vector.view(1, -1)
                    - expected_score_from_belief
                ),
                dim=1,
            )
        )
    ...
    def loss_shortterm_score_error_samplewise(
        self, pred, td_score_pred, td_score_target, weight, global_weight
    ):
    ...
    def accuracy1(self, pred_logits, target_probs, weight, global_weight):
        return torch.sum(
            global_weight
            * weight
            * (torch.argmax(pred_logits, dim=1) == torch.argmax(target_probs, dim=1))
        )
    ...
    def target_entropy(self, target_probs, weight, global_weight):
        return torch.sum(
            global_weight
            * weight
            * -torch.sum(target_probs * torch.log(target_probs + 1e-30), dim=-1)
        )
        ...
        return torch.sum(
            ...
            * torch.square(
                torch.sum(
                    ...
                    * constant_like([1, -1, 0], global_weight),
                    dim=1,
                )
            )
        )
    ...
    def get_specific_norms_and_gradient_stats(self, raw_model):
        with torch.no_grad():
            ...
            def add_norm_and_grad_stats(name):
                ...
                param_scale = (
                    torch.sqrt(torch.mean(torch.square(param)))
                ...
                orthograd = param.grad - param * (
                    torch.sum(param.grad * param)
                    / (1e-20 + torch.sum(torch.square(param)))
                )
    ...
    def metrics_dict_batchwise(
        ...
        if raw_model.get_has_intermediate_head():
            ...
            if raw_model.training:
                assert (
                    intermediate_loss_scale is not None
                    or intermediate_distill_scale is not None
                )
            else:
                if (
                    intermediate_loss_scale is None
                    and intermediate_distill_scale is None
                ):
            ...
            if intermediate_loss_scale is not None:
                ...
                for key, value in iresults.items():
                    if key != "loss_sum":
                        results["I" + key] = value
                results["loss_sum"] = (
                    results["loss_sum"] + intermediate_loss_scale * iresults["loss_sum"]
                )
            if intermediate_distill_scale is not None:
                ...
                results["loss_sum"] = (
                    results["loss_sum"]
                    + intermediate_distill_scale * iresults["loss_sum"]
                )
    ...
    def metrics_dict_batchwise_single_heads_output(
        ...
        target_policy_player = target_policy_player / torch.sum(
            target_policy_player, dim=1, keepdim=True
        )
        target_policy_opponent = target_policy_ncmove[:, 1, :]
        target_policy_opponent = target_policy_opponent / torch.sum(
            target_policy_opponent, dim=1, keepdim=True
        )
        ...
        target_policy_player_soft /= torch.sum(
            target_policy_player_soft, dim=1, keepdim=True
        )
        ...
        target_policy_opponent_soft /= torch.sum(
            target_policy_opponent_soft, dim=1, keepdim=True
        )
        ...
        target_td_value = torch.stack(
            (
                target_global_nc[:, 4:7],
                target_global_nc[:, 8:11],
                target_global_nc[:, 12:15],
            ),
            dim=1,
        )
        target_td_score = torch.cat(
            (
                target_global_nc[:, 7:8],
                target_global_nc[:, 11:12],
                target_global_nc[:, 15:16],
            ),
            dim=1,
        )
        ...
        seki_weight_scale = (
            seki_weight_scale.sum()
            if not isinstance(seki_weight_scale, float)
            else seki_weight_scale
        )
    ...
    def metrics_dict_self_distill(
        ...
        model_output_postprocessed_main = tuple(
            tensor.detach() for tensor in model_output_postprocessed_main
        )
        (
        ...
        target_score_distribution = torch.nn.functional.softmax(
            target_scorebelief_logits, dim=1
        )

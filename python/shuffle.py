...
def merge_shards(
    ...
    num_batches = (
        num_rows // (batch_size * ensure_batch_multiple)
    ) * ensure_batch_multiple
    if output_npz:
        start = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        required=False,
        help="Minimum training rows to use, default 250k",
    )
    parser.add_argument(
        "-max-rows",
        type=int,
        required=False,
        help="Maximum training rows to use, default unbounded",
    )
    parser.add_argument(
        "-keep-target-rows",
        type=int,
        required=False,
        help=(
            "Target number of rows to actually keep in the final data set, default 1.2M"
        ),
    )
    parser.add_argument(
        ...
        required=True,
        help=(
            "Beyond min rows, initially expand the window by this much every"
            " post-random data row"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-summary-file", required=False, help="Summary json file for directory contents"
    )
    ...
    parser.add_argument(
        "-ensure-batch-multiple",
        type=int,
        required=False,
        help="Ensure each file is a multiple of this many batches",
    )
    parser.add_argument(
        "-worker-group-size",
        type=int,
        required=False,
        help=(
            "Internally, target having many rows per parallel sharding worker (doesnt"
            " affect merge)"
        ),
    )
    parser.add_argument(
        "-exclude", required=False, help="Text file with npzs to ignore, one per line"
    )
    parser.add_argument(
        "-exclude-prefix",
        required=False,
        help="Prefix to concat to lines in exclude to produce the full file path",
    )
    parser.add_argument(
        "-exclude-basename",
        required=False,
        action="store_true",
        help="Consider an exclude to match if basename matches",
    )
    parser.add_argument(
        "-only-include-md5-path-prop-lbound",
        type=float,
        required=False,
        help="Just before sharding, include only filepaths hashing to float >= this",
    )
    parser.add_argument(
        "-only-include-md5-path-prop-ubound",
        type=float,
        required=False,
        help="Just before sharding, include only filepaths hashing to float < this",
    )
    parser.add_argument(
        ...
        action="store_true",
        required=False,
        help="Output results as npz files",
    )
    ...
    if min_rows is None:
        print(
            "NOTE: -min-rows was not specified, defaulting to requiring 250K rows"
            " before shuffling."
        )
        min_rows = 250000
    if keep_target_rows is None:
        print(
            "NOTE: -keep-target-rows was not specified, defaulting to keeping the first"
            " 1.5M rows."
        )
        print(
            "(slightly larger than default training epoch size of 1M, to give 1 epoch"
            " of data regardless of discreteness rows or batches per output file)"
        )
        print(
            "If you intended to shuffle the whole dataset instead, pass in"
            " -keep-target-rows <very large number>"
        )
        ...
        for d in dirs:
            for path, dirnames, filenames in os.walk(d, followlinks=True):
                ...
                while i < len(dirnames):
                    dirname = dirnames[i]
                    filename_mtime_num_rowss = summary_data_by_dirpath.get(
                    ...
                    if filename_mtime_num_rowss is not None:
                        ...
                        for filename, mtime, num_rows in filename_mtime_num_rowss:
                            ...
                            if (
                                exclude_basename
                                and os.path.basename(filename) in exclude_set
                            ):
                                excluded_count += 1
                            ...
                            if num_rows is None:
                                print(
                                    (
                                        "WARNING: Skipping bad rowless file, treating"
                                        " as exclude: "
                                    ),
                                    filename,
                                )
    ...
    print(
        "Total number of files with unknown row count: %d"
        % len(files_with_unknown_num_rows),
        flush=True,
    )
    print("Excluded count: %d" % excluded_count, flush=True)
    print(
        "Excluded count due to looking like temp file: %d" % tempfilelike_count,
        flush=True,
    )
    print(
        "Excluded count due to cmdline excludes file: %d"
        % excluded_due_to_excludes_count,
        flush=True,
    )
    ...
    def num_desired_rows():
        ...
        unscaled_power_law = (power_law_x**taper_window_exponent) - (
    ...
    desired_num_rows = (
        min(desired_num_rows, max_rows) if max_rows is not None else desired_num_rows
    )
    ...
    with TimeStuff("Computing desired rows"):
        for i in range(len(files_with_row_range)):
            ...
            if i % print_stride == 0 or num_rows_used >= desired_num_rows:
                print(
                    "Using: %s (%d-%d) (%d/%d desired rows)"
                    % (filename, start_row, end_row, num_rows_used, desired_num_rows)
                )
    ...
    print(
        "Finally, using: (%d-%d) (%d/%d desired rows)"
        % (min_start_row, max_end_row, num_rows_used, desired_num_rows)
    )
    ...
    if output_npz:
        out_files = [
            os.path.join(out_dir, "data%d.npz" % i) for i in range(num_out_files)
        ]
    else:
        assert False, "No longer supports outputting tensorflow data"

    out_tmp_dirs = [
        os.path.join(out_tmp_dir, "tmp.shuf%d" % i) for i in range(num_out_files)
    ]
    print(
        "Writing %d output files with %d kept / %d desired rows"
        % (num_out_files, approx_rows_to_keep, desired_num_rows)
    )
    ...
    if (
        only_include_md5_path_prop_lbound is not None
        or only_include_md5_path_prop_ubound is not None
    ):
        ...
        for input_file, num_rows_in_file in desired_input_files:
            ...
            hashfloat = (
                int(
                    "0x"
                    + hashlib.md5(str(input_file_base).encode("utf-8")).hexdigest()[
                        :13
                    ],
                    16,
                )
            ...
            if (
                only_include_md5_path_prop_lbound is not None
                and hashfloat < only_include_md5_path_prop_lbound
            ):
                ok = False
            if (
                only_include_md5_path_prop_ubound is not None
                and hashfloat >= only_include_md5_path_prop_ubound
            ):
                ok = False
            if ok:
        ...
        print(
            "Due to only_include_md5, filtering down to %d/%d files"
            % (len(new_desired_input_files), len(desired_input_files))
        )
    ...
    if group_size_so_far > 0:
        ...
        group_size_so_far = 0
    print(
        "Grouping %d input files into %d sharding groups"
        % (len(desired_input_files), len(desired_input_file_groups)),
        flush=True,
    )
        ...
        with TimeStuff("Sharding"):
            shard_results = pool.starmap(
                ...
                [
                    (
                        input_idx,
                        desired_input_file_groups[input_idx],
                        num_out_files,
                        out_tmp_dirs,
                        keep_prob,
                    )
    ...
    dump_value = {"range": (min_start_row, max_end_row)}

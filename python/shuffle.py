...
def joint_shuffle_take_first_n(n, arrs):
    ...
    perm = perm[:n]
    shuffled_arrs = [arr[perm] for arr in arrs]
...
def shardify(input_idx, input_file_group, num_out_files, out_tmp_dirs, keep_prob):
    ...
    for out_idx in range(num_out_files):
        ...
        np.savez_compressed(
            os.path.join(out_tmp_dirs[out_idx], f"{str(input_idx)}.npz"),
...
def merge_shards(filename, num_shards_to_merge, out_tmp_dir, batch_size, ensure_batch_multiple, output_npz):
    ...
    for input_idx in range(num_shards_to_merge):
        shard_filename = os.path.join(out_tmp_dir, f"{str(input_idx)}.npz")
    ...
    jsonfilename = f"{os.path.splitext(filename)[0]}.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-taper-window-scale", type=float, required=False, help="The scale at which the power law applies"
    )
    parser.add_argument(
        "-add-to-window", type=float, required=False, help="Compute as if the window size were this much larger/smaller"
    )
    ...
    parser.add_argument(
        "-ensure-batch-multiple", type=int, required=False, help="Ensure each file is a multiple of this many batches"
    )
    parser.add_argument(
        "-worker-group-size",
        type=int,
        required=False,
        help="Internally, target having many rows per parallel sharding worker (doesnt affect merge)",
    )
    parser.add_argument("-exclude", required=False, help="Text file with npzs to ignore, one per line")
    parser.add_argument(
        "-exclude-prefix", required=False, help="Prefix to concat to lines in exclude to produce the full file path"
    )
    parser.add_argument(
        "-exclude-basename",
        required=False,
    ...
    if keep_target_rows is None:
        ...
        print(
            "(slightly larger than default training epoch size of 1M, to give 1 epoch of data regardless of"
            " discreteness rows or batches per output file)"
        )
    ...
    print(
        "Finally, using: (%d-%d) (%d/%d desired rows)" % (min_start_row, max_end_row, num_rows_used, desired_num_rows)
    )
    ...
    if group_size_so_far > 0:
        ...
        group_size_so_far = 0
    print(
        "Grouping %d input files into %d sharding groups" % (len(desired_input_files), len(desired_input_file_groups)),
        flush=True,
    )
    ...
    with multiprocessing.Pool(num_processes) as pool:
        with TimeStuff("Sharding"):
            shard_results = pool.starmap(
                shardify,
                [
                    (input_idx, desired_input_file_groups[input_idx], num_out_files, out_tmp_dirs, keep_prob)

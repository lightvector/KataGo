...
class SgfDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        ...
        if worker_info is None:
            ...
            rand = random.Random(
                os.urandom(32) + "#SgfDataset#".encode() + str(worker_info.id).encode()
            )
        ...
        try:
            while True:
                ...
                error_count = 0
                print(
                    "Iterator beginning reading of files %d / %d"
                    % (file_count, len(files)),
                    flush=True,
                )
                for filename in files:
                    try:
                        (metadata, setup, moves, rules) = data.load_sgf_moves_exn(
                            filename
                        )
                    ...
                    if (
                        metadata.size != 19
                        or len(setup) != 0
                        or (metadata.handicap is not None and metadata.handicap != 0)
                    ):
                        continue
                    ...
                    for pla, loc in moves:
                        if rand.random() < self.sample_prob:
                            ...
                            result = torch.zeros(
                                (3,), dtype=torch.float32, device=cpudevice
                            )
                            aux = torch.zeros(
                            ...
                            (alwaysknownxmin, alwaysknownxmax) = random_subinterval(
                                rand, metadata.size
                            )
                            (alwaysknownymin, alwaysknownymax) = random_subinterval(
                                rand, metadata.size
                            )

                            if (
                                alwaysknownxmin <= 0
                                and alwaysknownxmax >= metadata.size - 1
                                and alwaysknownymin <= 0
                                and alwaysknownymax >= metadata.size - 1
                            ):
                                pass
                            else:
                                # Channel 1: On-board
                                ...
                                if (
                                    alwaysknownxmax < 0
                                    or alwaysknownxmin >= metadata.size
                                    or alwaysknownymax < 0
                                    or alwaysknownymin >= metadata.size
                                ):
                                    num_always_known_poses = 0
                                else:
                                    num_always_known_poses = (
                                        min(alwaysknownxmax, metadata.size - 1)
                                        - max(alwaysknownxmin, 0)
                                        + 1
                                    ) * (
                                        min(alwaysknownymax, metadata.size - 1)
                                        - max(alwaysknownymin, 0)
                                        + 1
                                    )
                                num_not_always_known_poses = (
                    ...
                    if file_count % 200 == 0:
                        print(
                            "Read through file %d / %d  (error count %d)"
...
if __name__ == "__main__":
  ...
  """
    ...
    parser.add_argument(
        "-testprop", help="Proportion of data for test", type=float, required=True
    )
    ...
    parser.add_argument(
        "-grad-clip-scale", help="Gradient clip multiplier", type=float, required=False
    )
    parser.add_argument(
        "-num-data-workers",
        ...
        for parent, subdirs, files in os.walk(datadir):
            for name in files:
                if name.endswith(".sgf"):
                    ...
                    r = float.fromhex(
                        "0."
                        + hashlib.md5(os.path.join(parent, name).encode()).hexdigest()[
                            :16
                        ]
                    )
                    if r < testprop:
    ...
    traindataset = ShuffledDataset(
        SgfDataset(
            trainfiles, max_turn, break_prob_per_turn, sample_prob=0.5, endless=True
        ),
        shuffle_buffer_size,
    )
    testdataset = SgfDataset(
        testfiles, max_turn, break_prob_per_turn, sample_prob=0.2, endless=True
    )
    ...
    def lossfunc(inputs, results, preds, aux, auxpreds):
        ...
        aux_loss = (
            -torch.sum(
                aux
                * F.log_softmax(auxpreds, dim=1)
                * inputs[:, 4:5, :, :]
                / torch.sum(inputs[:, 1:2, :, :], dim=[2, 3], keepdim=True)
            )
            * 0.3
        )
    ...
    if os.path.exists(modelpath):
        ...
        if model.num_channels != num_channels:
            raise Exception(
                "Number of channels in model is %d but command line arg was %d"
                % (model.num_channels, num_channels)
            )
        if model.num_blocks != num_blocks:
            raise Exception(
                "Number of blocks in model is %d but command line arg was %d"
    ...
    while True:
        for i in range(batches_per_epoch):
            ...
            if running_batch_count >= print_every_batches:
                trainlog(
                    "TRAIN samples: %d,  batches: %d,  main loss: %.5f,  aux loss:"
                    " %.5f,  gnorm: %.2f,  ewms_exgnorm: %.3g"
                    % (
        ...
        trainlog(
            "TEST samples %d,  main loss: %.5f,  aux loss %.5f"
            % (
                test_samples,
                test_main_loss / test_samples,
                test_aux_loss / test_samples,
            )
        )

...
class SgfDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        ...
        try:
                ...
                for filename in files:
                    ...
                    for pla, loc in moves:
                        if rand.random() < self.sample_prob:
                            inputs = torch.zeros(
                                (8, metadata.size, metadata.size), dtype=torch.float32, device=cpudevice
                            )
                            ...
                            if (
                                alwaysknownxmin > 0
                                or alwaysknownxmax < metadata.size - 1
                                or alwaysknownymin > 0
                                or alwaysknownymax < metadata.size - 1
                            ):
                                # Channel 1: On-board
                                inputs[1, :, :].fill_(1.0)
                        ...
                        try:
                            ...
                            print(f"Illegal move, skipping file {filename}:{str(e)}", flush=True)
                            break
                        turn_number += 1
                    ...
                    if file_count % 200 == 0:
                        print(
                            "Read through file %d / %d  (error count %d)" % (file_count, len(files), error_count),
                            flush=True,
                        )
            ...
            print(f"EXCEPTION IN GENERATOR: {str(e)}")
            traceback.print_exc()
            print("---", flush=True)
            yield e
...
if __name__ == "__main__":
  ...
  """
    ...
    traindataset = ShuffledDataset(
        SgfDataset(trainfiles, max_turn, break_prob_per_turn, sample_prob=0.5, endless=True), shuffle_buffer_size
    )
    testdataset = SgfDataset(testfiles, max_turn, break_prob_per_turn, sample_prob=0.2, endless=True)

    batch_size = 128
    trainloader = torch.utils.data.DataLoader(
        traindataset, batch_size=batch_size, shuffle=False, num_workers=num_data_workers, drop_last=True
    )
    testloader = torch.utils.data.DataLoader(
        testdataset, batch_size=batch_size, shuffle=False, num_workers=num_data_workers, drop_last=True
    )
    ...
    if os.path.exists(modelpath):
        ...
        if model.num_channels != num_channels:
            raise Exception(
                "Number of channels in model is %d but command line arg was %d" % (model.num_channels, num_channels)
            )
        if model.num_blocks != num_blocks:
            raise Exception(
                "Number of blocks in model is %d but command line arg was %d" % (model.num_blocks, num_blocks)
            )
    ...
    while True:
        for i in range(batches_per_epoch):
            ...
            if running_batch_count >= print_every_batches:
                trainlog(
                    "TRAIN samples: %d,  batches: %d,  main loss: %.5f,  aux loss: %.5f,  gnorm: %.2f,  ewms_exgnorm:"
                    " %.3g"
                    % (
        ...
        trainlog(
            ...
            % (test_samples, test_main_loss / test_samples, test_aux_loss / test_samples)
        )

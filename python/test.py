...
if __name__ == "__main__":
  ...
  """
    ...
    parser.add_argument(
        "-model-kind",
        help="If specified, use this model kind instead of config",
        required=False,
    )
    ...
    parser.add_argument(
        "-batch-size", help="Batch size to use for testing", type=int, required=True
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-max-batches",
        help="Maximum number of batches for testing",
        type=int,
        required=False,
    )
...
def main(args):
    ...
    if checkpoint_file is None:
        ...
        assert (model_kind is None) != (
            config_file is None
        ), "Must provide exactly one of -model-kind and -config if no checkpoint"
        ...
        model, swa_model, _ = load_model(
            checkpoint_file, use_swa, device=device, pos_len=pos_len, verbose=True
        )
    ...
    val_files = [
        os.path.join(npzdir, fname)
        for fname in os.listdir(npzdir)
    ...
    with torch.no_grad():
        ...
        for batch in data_processing_pytorch.read_npz_training_data(
            ...
            accumulate_metrics(
                val_metric_sums, val_metric_weights, metrics, batch_size, decay=1.0
            )
            ...
            if num_batches_tested % 5 == 0:
                ...
                if remainder in [1, 3, 5, 7, 9]:
                    ...
                    metrics["time/1ksamp"] = (
                        total_inference_time / num_samples_tested * 1000.0
                    )
                    log_metrics(
                        "STATS SO FAR: ",
                        val_metric_sums,

...
class Board:
    ...
    def is_adjacent(self, loc1, loc2):
        return (
            loc1 == loc2 + self.adj[0]
            or loc1 == loc2 + self.adj[1]
            or loc1 == loc2 + self.adj[2]
            or loc1 == loc2 + self.adj[3]
        )
    ...
    def is_simple_eye(self, pla, loc):
        ...
        if (
            (self.board[adj0] != pla and self.board[adj0] != Board.WALL)
            or (self.board[adj1] != pla and self.board[adj1] != Board.WALL)
            or (self.board[adj2] != pla and self.board[adj2] != Board.WALL)
            or (self.board[adj3] != pla and self.board[adj3] != Board.WALL)
        ):
            return False
                ...
                if head not in connectingGroupHeads:
                    ...
                    while True:
                        for k in range(4):
                            ...
                            if (
                                possibleLib != loc
                                and wouldBeEmpty(possibleLib)
                                and possibleLib not in libs
                            ):
    ...
    def to_string(self):
        ...
        return "\n".join(
            "".join(get_piece(x, y) for x in range(self.size)) for y in range(self.size)
        )
    ...
    def playRecordedUnsafe(self, pla, loc):
        ...
        for i in range(4):
            ...
            if (
                self.board[adj] == opp
                and self.group_liberty_count[self.group_head[adj]] == 1
            ):
                capDirs.append(i)
    ...
    def floodFillStonesHelper(self, head, tailTarget, loc, pla):
        ...
        for i in range(4):
            adj = loc + self.adj[i]
            if self.board[adj] == Board.EMPTY:
                nextTailTarget = self.floodFillStonesHelper(
                    head, nextTailTarget, adj, pla
                )
    ...
    def rebuildChainHelper(self, head, tailTarget, loc, pla):
        ...
        for dloc in self.adj:
            if self.board[loc + dloc] == Board.EMPTY and not self.is_group_adjacent(
                head, loc + dloc
            ):
    ...
    def add_unsafe(self, pla, loc):
        ...
        if self.board[adj2] == Board.BLACK or self.board[adj2] == Board.WHITE:
            if (
                self.group_head[adj2] != self.group_head[adj0]
                and self.group_head[adj2] != self.group_head[adj1]
            ):
                self.group_liberty_count[self.group_head[adj2]] -= 1
        if self.board[adj3] == Board.BLACK or self.board[adj3] == Board.WHITE:
            if (
                self.group_head[adj3] != self.group_head[adj0]
                and self.group_head[adj3] != self.group_head[adj1]
                and self.group_head[adj3] != self.group_head[adj2]
            ):
        ...
        if (
            self.board[adj0] == opp
            and self.group_liberty_count[self.group_head[adj0]] == 0
        ):
        ...
        if (
            self.board[adj1] == opp
            and self.group_liberty_count[self.group_head[adj1]] == 0
        ):
        ...
        if (
            self.board[adj2] == opp
            and self.group_liberty_count[self.group_head[adj2]] == 0
        ):
        ...
        if (
            self.board[adj3] == opp
            and self.group_liberty_count[self.group_head[adj3]] == 0
        ):
    ...
    def merge_unsafe(self, loc0, loc1):
        if (
            self.group_stone_count[self.group_head[loc0]]
            >= self.group_stone_count[self.group_head[loc1]]
        ):
            parent = loc0
            child = loc1
        else:
        ...
        while True:
            ...
            if self.board[adj0] == Board.EMPTY and not self.is_group_adjacent(
                phead, adj0
            ):
                new_liberties += 1
            if self.board[adj1] == Board.EMPTY and not self.is_group_adjacent(
                phead, adj1
            ):
                new_liberties += 1
            if self.board[adj2] == Board.EMPTY and not self.is_group_adjacent(
                phead, adj2
            ):
                new_liberties += 1
            if self.board[adj3] == Board.EMPTY and not self.is_group_adjacent(
                phead, adj3
            ):
                new_liberties += 1
    ...
    def searchIsLadderCaptured(self, loc, defenderFirst):
        ...
        )  # A bit bigger due to paranoia about recaptures making the sequence longer.
        ...
        while True:
            ...
            isDefender = (defenderFirst and (stackIdx % 2) == 0) or (
                not defenderFirst and (stackIdx % 2) == 1
            )
            ...
            if moveListCur[stackIdx] == -1:
                ...
                if isDefender:
                    ...
                    if (
                        ...
                        if (
                            self.get_liberties_after_play(pla, move0, 3) <= 2
    ...
    def calculateNonDameTouchingArea(
        self, result, keepTerritories, keepStones, isMultiStoneSuicideLegal
    ):
        ...
        for i in range(self.arrsize):
            result[i] = Board.EMPTY
        self.calculateAreaForPla(
            Board.BLACK, True, True, isMultiStoneSuicideLegal, basicArea
        )
        self.calculateAreaForPla(
            Board.WHITE, True, True, isMultiStoneSuicideLegal, basicArea
        )
            ...
            for y in range(self.size):
                for x in range(self.size):
                    ...
                    if (
                        ...
                        and basicArea[loc] != self.board[loc]
                    ):
    ...
    def calculateAreaForPla(
            ...
            if isMultiStoneSuicideLegal or self.board[loc] == Board.EMPTY:
                ...
                for i in range(oldVLen):
                    if isAdjacentToPlaHead(loc, vitalForPlaHeadsLists[vStart + i]):
                        vitalForPlaHeadsLists[vStart + newVLen] = vitalForPlaHeadsLists[
                            vStart + i
                        ]
                        newVLen += 1
                ...
                if vitalCountByPlaHead[plaHead] < 2:
                    ...
                    while True:
                        for j in range(4):
                            adj = cur + self.adj[j]
                            if self.board[adj] == Board.EMPTY or self.board[adj] == opp:
                                bordersNonPassAlivePlaByHead[regionHeadByLoc[adj]] = (
                                    True
                                )
        ...
        for i in range(numRegions):
            head = regionHeads[i]
            shouldMark = (
                numInternalSpacesMax2[i] <= 1
                and atLeastOnePla
                and not bordersNonPassAlivePlaByHead[head]
            )
            shouldMark = shouldMark or (
                safeBigTerritories
                and atLeastOnePla
                and not containsOpp[i]
                and not bordersNonPassAlivePlaByHead[head]
            )
            shouldMark = shouldMark or (
                unsafeBigTerritories and atLeastOnePla and not containsOpp[i]
            )
        ...
        for y in range(self.size):
            for x in range(self.size):
                ...
                if basicArea[loc] != Board.EMPTY and not isDameTouching[loc]:
                    ...
                    if (
                        (
                            self.board[loc + ADJ0] == Board.EMPTY
                            and basicArea[loc + ADJ0] == Board.EMPTY
                        )
                        or (
                            self.board[loc + ADJ1] == Board.EMPTY
                            and basicArea[loc + ADJ1] == Board.EMPTY
                        )
                        or (
                            self.board[loc + ADJ2] == Board.EMPTY
                            and basicArea[loc + ADJ2] == Board.EMPTY
                        )
                        or (
                            self.board[loc + ADJ3] == Board.EMPTY
                            and basicArea[loc + ADJ3] == Board.EMPTY
                        )
                    ):
                        pla = basicArea[loc]

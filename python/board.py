...
class Board:
    ...
    def __init__(self, size, copy_other=None):
        if size < 2 or size > 39:
            raise ValueError(f"Invalid board size: {str(size)}")
        self.size = size
    ...
    def num_liberties(self, loc):
        if self.board[loc] in [Board.EMPTY, Board.WALL]:
            return 0
    ...
    def is_simple_eye(self, pla, loc):
        ...
        if (
            self.board[adj0] not in [pla, Board.WALL]
            or self.board[adj1] not in [pla, Board.WALL]
            or self.board[adj2] not in [pla, Board.WALL]
            or self.board[adj3] not in [pla, Board.WALL]
        ):
            return False
        ...
        against_wall = (
            self.board[adj0] == Board.WALL
            or self.board[adj1] == Board.WALL
            or self.board[adj2] == Board.WALL
            or self.board[adj3] == Board.WALL
        )

        return not against_wall

    def would_be_legal(self, pla, loc):
        if pla not in [Board.BLACK, Board.WHITE]:
            return False
    ...
    def would_be_suicide(self, pla, loc):
        ...
        if (
            self.board[adj0] == Board.EMPTY
            or (self.board[adj0] == opp and self.group_liberty_count[self.group_head[adj0]] == 1)
            or self.board[adj1] == Board.EMPTY
            or (self.board[adj1] == opp and self.group_liberty_count[self.group_head[adj1]] == 1)
            or self.board[adj2] == Board.EMPTY
            or (self.board[adj2] == opp and self.group_liberty_count[self.group_head[adj2]] == 1)
            or self.board[adj3] == Board.EMPTY
            or (self.board[adj3] == opp and self.group_liberty_count[self.group_head[adj3]] == 1)
        ):
            return False
        # If connects to own stone with enough liberties, then not suicide
        if (
            self.board[adj0] == pla
            and self.group_liberty_count[self.group_head[adj0]] > 1
            or self.board[adj1] == pla
            and self.group_liberty_count[self.group_head[adj1]] > 1
            or self.board[adj2] == pla
            and self.group_liberty_count[self.group_head[adj2]] > 1
            or self.board[adj3] == pla
            and self.group_liberty_count[self.group_head[adj3]] > 1
        ):
            return False
        return True
    ...
    def would_be_single_stone_suicide(self, pla, loc):
        ...
        if self.board[adj0] == pla or self.board[adj1] == pla or self.board[adj2] == pla or self.board[adj3] == pla:
            return False
        return True
    ...
    def to_string(self):
        def get_piece(x, y):
            ...
            if self.board[loc] == Board.BLACK:
                ...
                return "O "
            elif x in [3, self.size / 2, self.size - 1 - 3] and y in [
                3,
                self.size / 2,
                self.size - 1 - 3,
            ]:
    ...
    def to_liberty_string(self):
        def get_piece(x, y):
            loc = self.loc(x, y)
            if self.board[loc] in [Board.BLACK, Board.WHITE]:
    ...
    def set_stone(self, pla, loc):
        if pla not in [Board.EMPTY, Board.BLACK, Board.WHITE]:
    ...
    def playRecordedUnsafe(self, pla, loc):
        ...
        # Suicide
        selfCap = self.board[loc] == Board.EMPTY
    ...
    def add_unsafe(self, pla, loc):
        ...
        liberties = sum(self.board[loc + dloc] == Board.EMPTY for dloc in self.adj)
        ...
        adj2 = loc + self.adj[2]
        adj3 = loc + self.adj[3]
        if self.board[adj0] in [Board.BLACK, Board.WHITE]:
            self.group_liberty_count[self.group_head[adj0]] -= 1
        if self.board[adj1] in [Board.BLACK, Board.WHITE] and self.group_head[adj1] != self.group_head[adj0]:
            self.group_liberty_count[self.group_head[adj1]] -= 1
        if self.board[adj2] in [Board.BLACK, Board.WHITE] and self.group_head[adj2] not in [
            self.group_head[adj0],
            self.group_head[adj1],
        ]:
            self.group_liberty_count[self.group_head[adj2]] -= 1
        if self.board[adj3] in [Board.BLACK, Board.WHITE] and self.group_head[adj3] not in [
            self.group_head[adj0],
            self.group_head[adj1],
            self.group_head[adj2],
        ]:
    ...
    def changeSurroundingLiberties(self, loc, pla, delta):
        ...
        if self.board[adj1] == pla and self.group_head[adj1] != self.group_head[adj0]:
            self.group_liberty_count[self.group_head[adj1]] += delta
        if self.board[adj2] == pla and self.group_head[adj2] not in [
            self.group_head[adj0],
            self.group_head[adj1],
        ]:
            self.group_liberty_count[self.group_head[adj2]] += delta
        if self.board[adj3] == pla and self.group_head[adj3] not in [
            self.group_head[adj0],
            self.group_head[adj1],
            self.group_head[adj2],
        ]:
    ...
    def is_group_adjacent(self, head, loc):
        return (
            ...
            or self.group_head[loc + self.adj[1]] == head
            or self.group_head[loc + self.adj[2]] == head
            or self.group_head[loc + self.adj[3]] == head
        )
    ...
    def remove_unsafe(self, group):
        ...
        while True:
            ...
            if self.board[adj0] == opp:
                self.group_liberty_count[self.group_head[adj0]] += 1
            if self.board[adj1] == opp and self.group_head[adj1] != self.group_head[adj0]:
                self.group_liberty_count[self.group_head[adj1]] += 1
            if self.board[adj2] == opp and self.group_head[adj2] not in [
                self.group_head[adj0],
                self.group_head[adj1],
            ]:
                self.group_liberty_count[self.group_head[adj2]] += 1
            if self.board[adj3] == opp and self.group_head[adj3] not in [
                self.group_head[adj0],
                self.group_head[adj1],
                self.group_head[adj2],
            ]:
    ...
    def findLiberties(self, loc, buf):
        ...
        while True:
            for i in range(4):
                lib = cur + self.adj[i]
                if self.board[lib] == Board.EMPTY and lib not in buf:
                    buf.append(lib)
        ...
        while True:
            for i in range(4):
                ...
                if self.board[adj] == opp:
                    ...
                    if self.group_liberty_count[head] == 1 and head not in chainHeadsChecked:
    ...
    def wouldBeKoCapture(self, loc, pla):
        ...
        for i in range(4):
            adj = loc + self.adj[i]
            if self.board[adj] not in [Board.WALL, opp]:
                return False
            if self.board[adj] == opp and self.group_liberty_count[self.group_head[adj]] == 1:
    ...
    def searchIsLadderCapturedAttackerFirst2Libs(self, loc):
        if not self.is_on_board(loc):
            return []
        if self.board[loc] not in [Board.BLACK, Board.WHITE]:
            return []
    ...
    def searchIsLadderCaptured(self, loc, defenderFirst):
        ...
        moveLists = [[] for _ in range(arrSize)]
        # Current move list idx searched, equal to -1 if list has not been generated.
        moveListCur = [0 for _ in range(arrSize)]
        # Records so that we can undo moves as we search back up.
        records = [None for _ in range(arrSize)]
        stackIdx = 0
            ...
            if moveListCur[stackIdx] == -1:
                ...
                if isDefender:
                    ...
                    if (
                        libs0 == 0
                        and libs1 == 0
                        and self.wouldBeKoCapture(move0, opp)
                        and self.wouldBeKoCapture(move1, opp)
                        and self.get_liberties_after_play(pla, move0, 3) <= 2
                        and self.get_liberties_after_play(pla, move1, 3) <= 2
                        and self.hasLibertyGainingCaptures(loc)
                    ):
                        returnValue = True
                        returnedFromDeeper = True
                        stackIdx -= 1
    ...
    def calculateArea(
        self, result, nonPassAliveStones, safeBigTerritories, unsafeBigTerritories, isMultiStoneSuicideLegal
    ):
        for i in range(self.arrsize):
            result[i] = Board.EMPTY
        self.calculateAreaForPla(
            Board.BLACK, safeBigTerritories, unsafeBigTerritories, isMultiStoneSuicideLegal, result
        )
        self.calculateAreaForPla(
            Board.WHITE, safeBigTerritories, unsafeBigTerritories, isMultiStoneSuicideLegal, result
        )
    ...
    def calculateNonDameTouchingArea(self, result, keepTerritories, keepStones, isMultiStoneSuicideLegal):
        ...
        basicArea = [Board.EMPTY for _ in range(self.arrsize)]
        for i in range(self.arrsize):
            result[i] = Board.EMPTY
        ...
        if keepTerritories:
            for y in range(self.size):
                for x in range(self.size):
                    loc = self.loc(x, y)
                    if basicArea[loc] not in [Board.EMPTY, self.board[loc]]:
    ...
    def calculateAreaForPla(self, pla, safeBigTerritories, unsafeBigTerritories, isMultiStoneSuicideLegal, result):
        ...
        def buildRegion(head, tailTarget, loc, regionIdx):
            ...
            for i in range(4):
                adj = loc + self.adj[i]
                if self.board[adj] in [Board.EMPTY, opp]:
        ...
        for i in range(numRegions):
            ...
            shouldMark = shouldMark or (
                safeBigTerritories and atLeastOnePla and not containsOpp[i] and not bordersNonPassAlivePlaByHead[head]
            )
    ...
    def calculateNonDameTouchingAreaHelper(self, basicArea, result):
        ...
        for y in range(self.size):
            for x in range(self.size):
                loc = self.loc(x, y)
                if (
                    basicArea[loc] != Board.EMPTY
                    and not isDameTouching[loc]
                    and (
                        (self.board[loc + ADJ0] == Board.EMPTY and basicArea[loc + ADJ0] == Board.EMPTY)
                        or (self.board[loc + ADJ1] == Board.EMPTY and basicArea[loc + ADJ1] == Board.EMPTY)
                        or (self.board[loc + ADJ2] == Board.EMPTY and basicArea[loc + ADJ2] == Board.EMPTY)
                        or (self.board[loc + ADJ3] == Board.EMPTY and basicArea[loc + ADJ3] == Board.EMPTY)
                    )
                ):
                    pla = basicArea[loc]

...
parser.add_argument(
    "-model-zip", help="zipped model file with tf weights", required=True
)
parser.add_argument(
    "-upload-log-file", help="log upload data to this file", required=True
)
parser.add_argument(
    "-metadata-file", help="metadata.json file for recording some stats", required=False
)
parser.add_argument(
    "-parents-dir",
    help="dir with uploaded models dirs for finding parent",
    required=False,
)
parser.add_argument(
    "-connection-config",
    help="config with serverUrl and username and password",
    required=True,
)
parser.add_argument(
    "-not-enabled",
...
parser.add_argument(
    "-rating-only",
    help="upload for rating only or not",
    type=int,
    default=0,
    required=False,
)
    ...
    with open(model_zip, "rb") as model_zip_handle:
        ...
        if parent_network_name_without_run is not None:
            data["parent_network"] = (
                ...
                + run_name
                + "-"
                + parent_network_name_without_run
                + "/",
            )
        ...
        if sslVerificationHost is not None:
            ...
            if sslVerifyPemPath is not None:
                ...
                result = requests.post(
                    url, files=data, auth=HTTPBasicAuth(username, password)
                )
...
if result.status_code == 400 and str(result.text).find("already exist") != -1:
    log(
        "Got 400 error with substring 'already exist', network already uploaded? So"
        " assuming everything is good"
    )
    write_log()
elif (
    result.status_code == 200 or result.status_code == 201 or result.status_code == 202
):
    log("Post success")
    write_log()
else:

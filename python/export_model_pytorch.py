...
def main(args):
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(f"{export_dir}/log.txt"),
        ],
    )
    ...
    extension = ".bin"
    mode = "wb"
    with open(f"{export_dir}/{filename_prefix}{extension}", mode) as f:
        ...
        def write_normactconv(name, normactconv):
            if normactconv.c_gpool is None:
                ...
                if normactconv.conv1x1 is None:
                    write_bn(f"{name}.norm", normactconv.norm)
                    write_activation(f"{name}.act", normactconv.act)
                    write_conv(f"{name}.conv", normactconv.conv)
                else:
                    write_bn(f"{name}.norm", normactconv.norm)
                    write_activation(f"{name}.act", normactconv.act)
                    ...
                    write_conv_weight(f"{name}.conv", combined_conv)
            else:
                assert normactconv.convpool is not None
                assert normactconv.conv1x1 is None
                write_bn(f"{name}.norm", normactconv.norm)
                write_activation(f"{name}.act", normactconv.act)
                write_conv(f"{name}.convpool.conv1r", normactconv.convpool.conv1r)
                write_conv(f"{name}.convpool.conv1g", normactconv.convpool.conv1g)
                write_bn(f"{name}.convpool.normg", normactconv.convpool.normg)
                write_activation(f"{name}.convpool.actg", normactconv.convpool.actg)
                write_matmul(f"{name}.convpool.linear_g", normactconv.convpool.linear_g.weight)
        ...
        def write_block(name, block):
            if isinstance(block, ResBlock) and block.normactconv1.c_gpool is None:
                ...
                writeln(name)
                write_normactconv(f"{name}.normactconv1", block.normactconv1)
                write_normactconv(f"{name}.normactconv2", block.normactconv2)
                ...
                write_normactconv(f"{name}.normactconvp", block.normactconvp)
                for i, subblock in enumerate(block.blockstack):
                    write_block(f"{name}.blockstack.{str(i)}", subblock)
                write_normactconv(f"{name}.normactconvq", block.normactconvq)
            else:
        ...
        def write_trunk(name, model):
            ...
            for i, block in enumerate(model.blocks):
                write_block(f"model.blocks.{str(i)}", block)
        ...
        def write_policy_head(name, policyhead):
            writeln(name)
            write_conv(f"{name}.conv1p", policyhead.conv1p)
            write_conv(f"{name}.conv1g", policyhead.conv1g)
            write_biasmask(f"{name}.biasg", policyhead.biasg)
            write_activation(f"{name}.actg", policyhead.actg)
            write_matmul(f"{name}.linear_g", policyhead.linear_g.weight)
            assert policyhead.linear_g.bias is None
            write_biasmask(f"{name}.bias2", policyhead.bias2)
            write_activation(f"{name}.act2", policyhead.act2)

            # Write only the this-move prediction, not the next-move prediction
            assert policyhead.conv2p.weight.shape[0] == 4
            write_conv_weight(f"{name}.conv2p", policyhead.conv2p.weight[:1])
            assert policyhead.conv2p.bias is None

            assert policyhead.linear_pass.weight.shape[0] == 4
            write_matmul(f"{name}.linear_pass", policyhead.linear_pass.weight[:1])
            assert policyhead.linear_pass.bias is None

        def write_value_head(name, valuehead):
            writeln(name)
            write_conv(f"{name}.conv1", valuehead.conv1)
            write_biasmask(f"{name}.bias1", valuehead.bias1)
            write_activation(f"{name}.act1", valuehead.act1)
            write_matmul(f"{name}.linear2", valuehead.linear2.weight)
            write_matbias(f"{name}.bias2", valuehead.linear2.bias)
            write_activation(f"{name}.act2", valuehead.act2)
            write_matmul(f"{name}.linear_valuehead", valuehead.linear_valuehead.weight)
            write_matbias(f"{name}.bias_valuehead", valuehead.linear_valuehead.bias)

            # For now, only output the scoremean and scorestdev and lead and vtime channels
            w = valuehead.linear_miscvaluehead.weight[:4]
            b = valuehead.linear_miscvaluehead.bias[:4]
            # Grab the shortterm channels
            w2 = valuehead.linear_moremiscvaluehead.weight[:2]
            b2 = valuehead.linear_moremiscvaluehead.bias[:2]
            w = torch.cat((w, w2), dim=0)
            b = torch.cat((b, b2), dim=0)
            write_matmul(f"{name}.linear_miscvaluehead", w)
            write_matbias(f"{name}.bias_miscvaluehead", b)

            write_conv(f"{name}.conv_ownership", valuehead.conv_ownership)
    ...
    logging.info(f"{str(datetime.datetime.now(datetime.timezone.utc))} UTC")

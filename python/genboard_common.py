...
class ResBlock(nn.Module):
    def __init__(self, num_channels, scale_init):
        ...
        torch.nn.init.normal_(
            self.conva.weight,
            std=math.sqrt(2.0 / num_channels / kernel_size / kernel_size) * scale_init,
        )
...
class GPoolResBlock(nn.Module):
    def __init__(self, num_channels, scale_init):
        ...
        torch.nn.init.normal_(
            self.conva.weight,
            std=math.sqrt(1.0 / num_channels / kernel_size / kernel_size) * scale_init,
        )
        ...
        torch.nn.init.normal_(
            self.convg.weight,
            std=math.sqrt(1.0 / num_channels / kernel_size / kernel_size)
            * math.sqrt(scale_init),
        )
        self.matg = nn.Parameter(torch.zeros(num_channels, num_channels))
        torch.nn.init.normal_(
            self.matg, std=math.sqrt(1.0 / num_channels) * math.sqrt(scale_init)
        )
        self.biasb = nn.Parameter(torch.zeros(num_channels, 1, 1))
        self.scalb = nn.Parameter(torch.ones(num_channels, 1, 1))
        self.convb = nn.Conv2d(
...
class Model(nn.Module):
    def __init__(self, num_channels, num_blocks):
        ...
        for b in range(num_blocks - 2):
            if next_is_gpool:
                self.blocks.append(
                    GPoolResBlock(self.num_channels, self.fixup_scale_init)
                )
            else:
                self.blocks.append(ResBlock(self.num_channels, self.fixup_scale_init))
    ...
    def forward(self, inputs):
        ...
        x = torch.sum(
            head_focus
            * inputs[:, self.inference_channel : self.inference_channel + 1, :, :],
            (2, 3),
        )

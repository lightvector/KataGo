...
class ResBlock(nn.Module):
    def __init__(self, num_channels, scale_init):
        ...
        self.conva = nn.Conv2d(
            in_channels=num_channels, out_channels=num_channels, kernel_size=kernel_size, padding=1, bias=False
        )
        torch.nn.init.normal_(
            self.conva.weight, std=math.sqrt(2.0 / num_channels / kernel_size / kernel_size) * scale_init
        )
...
class GPoolResBlock(nn.Module):
    def __init__(self, num_channels, scale_init):
        ...
        torch.nn.init.normal_(
            self.conva.weight, std=math.sqrt(1.0 / num_channels / kernel_size / kernel_size) * scale_init
        )
        self.convg = nn.Conv2d(
            in_channels=num_channels, out_channels=num_channels, kernel_size=kernel_size, padding=1, bias=False
        )
        torch.nn.init.normal_(
            self.convg.weight, std=math.sqrt(1.0 / num_channels / kernel_size / kernel_size) * math.sqrt(scale_init)
        )
...
class Model(nn.Module):
    def __init__(self, num_channels, num_blocks):
        ...
        self.convg = nn.Conv2d(
            in_channels=self.num_channels, out_channels=self.num_channels, kernel_size=1, padding=0, bias=False
        )

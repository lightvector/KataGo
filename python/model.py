...
class Model:
  ...
  def build_model(self,config,placeholders):
    ...
    scoring_output = self.conv_only_block("vscoring",v1_layer,diam=1,in_channels=v1_num_channels,out_channels=1, scale_initial_weights=0.2, reg=False) * mask
...
class Target_vars:
  def __init__(self,model,for_optimization,placeholders):
    ...
    self.seki_loss_unreduced = 0.25 * self.ownership_target_weight * (
      tf.reduce_sum(
        tf.nn.softmax_cross_entropy_with_logits_v2(

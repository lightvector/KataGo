...
class Model:
    ...
    def __init__(self, config, pos_len, placeholders, is_training=False):
        ...
        self.num_global_input_features = modelconfigs.get_num_global_input_features(
            config
        )

        self.bin_input_shape = [
            self.pos_len * self.pos_len,
            self.num_bin_input_features,
        ]
        self.binp_input_shape = [
            self.num_bin_input_features,
            (self.pos_len * self.pos_len + 7) // 8,
        ]
        self.global_input_shape = [self.num_global_input_features]
        self.post_input_shape = [
            self.pos_len,
            self.pos_len,
            self.num_bin_input_features,
        ]
        ...
        self.miscvalues_target_shape = [
            10
        ]  # 0:scoremean, #1 scorestdev, #2 lead, #3 variance time #4-#9 td value targets
        self.scoremean_target_shape = []  # 0
        self.scorestdev_target_shape = []  # 1
        self.lead_target_shape = []  # 2
        self.variance_time_target_shape = []  # 3
        self.moremiscvalues_target_shape = [
            8
        ]  # 0:shortterm value stdev, #1 shortterm score stdev, #2,3,4 td value, #5,#6-#7 td score
        self.scorebelief_target_shape = [
            self.pos_len * self.pos_len * 2 + Model.EXTRA_SCORE_DISTR_RADIUS * 2
        ]
        ...
        if (
            "support_japanese_rules" in config
            and config["support_japanese_rules"] == False
        ):
            self.support_japanese_rules = False

        self.use_scoremean_as_lead = False  # by default
        if (
            "use_scoremean_as_lead" in config
            and config["use_scoremean_as_lead"] == True
        ):
    ...
    def assert_batched_shape(self, name, tensor, shape):
        if len(tensor.shape) != len(shape) + 1 or [
            int(tensor.shape[i + 1].value) for i in range(len(shape))
        ] != [int(x) for x in shape]:
            raise Exception(
                "%s should have shape %s after a batch dimension but instead it had"
                " shape %s"
                % (name, str(shape), str([str(x.value) for x in tensor.shape]))
            )

    def assert_shape(self, name, tensor, shape):
        if len(tensor.shape) != len(shape) or [int(x.value) for x in tensor.shape] != [
            int(x) for x in shape
        ]:
            raise Exception(
                "%s should have shape %s but instead it had shape %s"
                % (name, str(shape), str([str(x.value) for x in tensor.shape]))
            )
    ...
    def batchnorm_and_mask(
        self, name, tensor, mask, mask_sum, use_gamma_in_fixup=False
    ):
        if self.use_fixup:
            self.batch_norms[name] = (
            ...
            if use_gamma_in_fixup:
                gamma = self.weight_variable_init_constant(
                    name + "/gamma", [tensor.shape[3].value], 1.0
                )
                beta = self.weight_variable_init_constant(
                    name + "/beta", [tensor.shape[3].value], 0.0, reg="tiny"
                )
        ...
        moving_mean = tf.compat.v1.get_variable(
            initializer=tf.zeros([num_channels]),
            name=(name + "/moving_mean"),
            trainable=False,
            collections=collections,
        )
        moving_var = tf.compat.v1.get_variable(
            initializer=tf.ones([num_channels]),
            name=(name + "/moving_variance"),
            trainable=False,
            collections=collections,
        )
        beta = self.weight_variable_init_constant(
            name + "/beta", [tensor.shape[3].value], 0.0, reg=False
        )
        ...
        return (
            tf.nn.batch_normalization(tensor, use_mean, use_var, beta, None, epsilon)
            * mask
        )
    ...
    def apply_symmetry(self, tensor, symmetries, inverse):
        ...
        if not inverse:
            tensor = tf.cond(ud, lambda: tf.reverse(tensor, [1]), lambda: tensor)
            tensor = tf.cond(lr, lambda: tf.reverse(tensor, [2]), lambda: tensor)

        tensor = tf.cond(
            transp, lambda: tf.transpose(tensor, [0, 2, 1, 3]), lambda: tensor
        )
    ...
    def conv_weight_variable(
        ...
        weights = self.weight_variable(
            name,
            [diam1, diam2, in_channels, out_channels],
            in_channels * diam1 * diam2,
            out_channels,
    ...
    def conv_block(
        ...
        weights = self.conv_weight_variable(
            name + "/w", diam, diam, in_channels, out_channels, scale_initial_weights
        )
        convolved = self.conv2d(in_layer, weights)
        self.outputs_by_layer.append((name + "/prenorm", convolved))
        out_layer = self.relu(
            name + "/relu",
            self.batchnorm_and_mask(name + "/norm", convolved, mask, mask_sum),
        )
    ...
    def res_conv_block(
        ...
        trans1_layer = self.relu(
            name + "/relu1",
            (self.batchnorm_and_mask(name + "/norm1", in_layer, mask, mask_sum)),
        )
        ...
        weights1 = self.conv_weight_variable(
            name + "/w1",
            diam,
            diam,
            main_channels,
            mid_channels,
            scale_initial_weights * fixup_scale,
        )
        conv1_layer = self.conv2d(trans1_layer, weights1)
        self.outputs_by_layer.append((name + "/conv1", conv1_layer))

        trans2_layer = self.relu(
            name + "/relu2",
            (
                self.batchnorm_and_mask(
                    name + "/norm2",
                    conv1_layer,
                    mask,
                    mask_sum,
                    use_gamma_in_fixup=True,
                )
            ),
        )
        ...
        weights2 = self.conv_weight_variable(
            name + "/w2",
            diam,
            diam,
            mid_channels,
            main_channels,
            scale_initial_weights * fixup_scale_last_layer,
        )
    ...
    def global_res_conv_block(
        ...
        weights1a = self.conv_weight_variable(
            name + "/w1a",
            diam,
            diam,
            main_channels,
            mid_channels,
            scale_initial_weights * fixup_scale2,
        )
        weights1b = self.conv_weight_variable(
            name + "/w1b",
            diam,
            diam,
            main_channels,
            global_mid_channels,
            scale_initial_weights * fixup_scale4,
        )
        ...
        trans1b_layer = self.relu(
            name + "/trans1b",
            (self.batchnorm_and_mask(name + "/norm1b", conv1b_layer, mask, mask_sum)),
        )
        trans1b_pooled = self.global_pool(trans1b_layer, mask_sum_hw, mask_sum_hw_sqrt)

        remix_weights = self.weight_variable(
            name + "/w1r",
            [global_mid_channels * 3, mid_channels],
            global_mid_channels * 3,
            mid_channels,
            scale_initial_weights * fixup_scale4 * 0.5,
        )
        conv1_layer = conv1a_layer + tf.tensordot(
            trans1b_pooled, remix_weights, axes=[[3], [0]]
        )
    ...
    def dilated_res_conv_block(
        ...
        weights2 = self.conv_weight_variable(
            ...
            diam,
            diam,
            mid_channels + dilated_mid_channels,
            main_channels,
            scale_initial_weights * fixup_scale_last_layer,
        )
        conv2_layer = self.conv2d(trans2_layer, weights2)
    ...
    def build_model(self, config, placeholders):
        ...
        # Input layer---------------------------------------------------------------------------------
        bin_inputs = (
            placeholders["bin_inputs"]
            if "bin_inputs" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + self.bin_input_shape, name="bin_inputs"
            )
        )
        global_inputs = (
            placeholders["global_inputs"]
            if "global_inputs" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + self.global_input_shape, name="global_inputs"
            )
        )
        symmetries = (
            placeholders["symmetries"]
            if "symmetries" in placeholders
            else tf.compat.v1.placeholder(tf.bool, [3], name="symmetries")
        )
        include_history = (
            placeholders["include_history"]
            if "include_history" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + [5], name="include_history"
            )
        )

        self.assert_batched_shape("bin_inputs", bin_inputs, self.bin_input_shape)
        self.assert_batched_shape(
            "global_inputs", global_inputs, self.global_input_shape
        )
        ...
        h0 = np.zeros(
            [self.num_bin_input_features, self.num_bin_input_features], dtype=np.float32
        )
        ...
        hist_matrix_base = tf.reshape(
            tf.constant(hist_matrix_base),
            [1, self.num_bin_input_features, self.num_bin_input_features],
        )
        ...
        hist_filter_matrix = hist_matrix_base + tf.tensordot(
            include_history, hist_matrix_builder, axes=[[1], [0]]
        )  # [batch,move] * [move,inc,outc] = [batch,inc,outc]
        cur_layer = tf.reshape(
            cur_layer, [-1, self.pos_len * self.pos_len, self.num_bin_input_features]
        )  # [batch,xy,inc]
        cur_layer = tf.matmul(
            cur_layer, hist_filter_matrix
        )  # [batch,xy,inc] * [batch,inc,outc] = [batch,xy,outc]
        cur_layer = tf.reshape(
            cur_layer, [-1, self.pos_len, self.pos_len, self.num_bin_input_features]
        )

        assert include_history.shape[1].value == 5
        transformed_global_inputs = global_inputs * tf.pad(
            include_history,
            [
                (0, 0),
                (0, self.num_global_input_features - include_history.shape[1].value),
            ],
            constant_values=1.0,
        )
        ...
        if "use_initial_conv_3" in config and config["use_initial_conv_3"]:
            trunk = self.conv_only_block(
                "conv1",
                cur_layer,
                diam=3,
                in_channels=input_num_channels,
                out_channels=trunk_num_channels,
            )
        ...
        ginputw = self.weight_variable(
            "ginputw",
            [self.num_global_input_features, trunk_num_channels],
            self.num_global_input_features * 2,
            trunk_num_channels,
        )
        ...
        self.initial_matmul = (
            "ginputw",
            self.num_global_input_features,
            trunk_num_channels,
        )
        ...
        for i in range(len(block_kind)):
            ...
            if kind == "regular":
                residual = self.res_conv_block(
                    ...
                    mask_sum,
                    diam=3,
                    main_channels=trunk_num_channels,
                    mid_channels=mid_num_channels,
                )
                trunk = self.merge_residual(name, trunk, residual)
                self.blocks.append(
                    ("ordinary_block", name, 3, trunk_num_channels, mid_num_channels)
                )
            elif kind == "dilated":
                residual = self.dilated_res_conv_block(
                    ...
                    diam=3,
                    main_channels=trunk_num_channels,
                    mid_channels=regular_num_channels,
                    dilated_mid_channels=dilated_num_channels,
                    dilation=2,
                )
                trunk = self.merge_residual(name, trunk, residual)
                self.blocks.append(
                ...
                residual = self.global_res_conv_block(
                    ...
                    main_channels=trunk_num_channels,
                    mid_channels=regular_num_channels,
                    global_mid_channels=gpool_num_channels,
                )
                trunk = self.merge_residual(name, trunk, residual)
                self.blocks.append(
                    (
        ...
        trunk = self.relu(
            "trunk/relu", (self.batchnorm_and_mask("trunk/norm", trunk, mask, mask_sum))
        )
        ...
        p1_intermediate_conv = self.conv_only_block(
            "p1/intermediate_conv",
            p0_layer,
            diam=1,
            in_channels=trunk_num_channels,
            out_channels=p1_num_channels,
        )
        ...
        g1_layer = self.conv_block(
            ...
            diam=1,
            in_channels=trunk_num_channels,
            out_channels=g1_num_channels,
        )
        self.g1_conv = ("g1", 1, trunk_num_channels, g1_num_channels)

        # Fold g1 down to single values for the board.
        g2_layer = self.global_pool(
            g1_layer, mask_sum_hw, mask_sum_hw_sqrt
        )  # shape [b,1,1,3*g1_num_channels]
        ...
        matmulg2w = self.weight_variable(
            "matmulg2w",
            [g2_num_channels, p1_num_channels],
            g2_num_channels * 4,
            p1_num_channels,
        )
        ...
        p1_layer = self.relu(
            "p1/relu",
            self.batchnorm_and_mask("p1/norm", p1_intermediate_sum, mask, mask_sum),
        )
        self.outputs_by_layer.append(("p1", p1_layer))

        # Finally, apply linear convolution to produce final output
        p2_layer = self.conv_only_block(
            "p2",
            p1_layer,
            diam=1,
            in_channels=p1_num_channels,
            out_channels=2,
            scale_initial_weights=0.3,
        )
        p2_layer = (
            p2_layer - (1.0 - mask) * 5000.0
        ...
        policy_output = tf.reshape(
            policy_output, [-1] + self.policy_output_shape_nopass
        )

        # Add pass move based on the global g values
        matmulpass = self.weight_variable(
            "matmulpass",
            [g2_num_channels, 2],
            g2_num_channels * 8,
            2,
        ...
        policy_output = tf.concat(
            [policy_output, pass_output], axis=1, name="policy_output"
        )
        ...
        v2b = self.weight_variable(
            "v2/b",
            [v2_size],
            v1_size * 3,
            v2_size,
            scale_initial_weights=0.2,
            reg="tiny",
        )
        v2_layer = self.relu_non_spatial(
            "v2/relu", tf.matmul(v1_layer_pooled, v2w) + v2b
        )
        ...
        v3b = self.weight_variable(
            "v3/b", [v3_size], v2_size, v3_size, scale_initial_weights=0.2, reg="tiny"
        )
        ...
        value_output = tf.reshape(
            v3_layer, [-1] + self.value_target_shape, name="value_output"
        )

        miscvalues_output = tf.reshape(
            mv3_layer, [-1] + self.miscvalues_target_shape, name="miscvalues_output"
        )
        moremiscvalues_output = tf.reshape(
            mmv3_layer,
            [-1] + self.moremiscvalues_target_shape,
        ...
        def scaletransform(tensor):
            ...
            return tf.where(
                tensor > 0,
                1.0 + tf.math.log(abstensor + 1.0),
                1.0 / (1.0 + tf.math.log(abstensor + 1.0)),
            )
        ...
        self.score_belief_offset_vector = np.array(
            [float(i - scorebelief_mid) + 0.5 for i in range(scorebelief_len)],
            dtype=np.float32,
        )
        self.score_belief_parity_vector = np.array(
            [0.5 - float((i - scorebelief_mid) % 2) for i in range(scorebelief_len)],
            dtype=np.float32,
        )
        sbv2_size = config["sbv2_num_channels"]
        sb2w = self.weight_variable(
            "sb2/w", [v1_size * 3, sbv2_size], v1_size * 3 + 1, sbv2_size
        )
        sb2b = self.weight_variable(
            "sb2/b",
            [sbv2_size],
            v1_size * 3 + 1,
            sbv2_size,
        ...
        sb2_offset_vector = tf.constant(
            0.05 * self.score_belief_offset_vector, dtype=tf.float32
        )
        sb2_parity_vector = (
            tf.reshape(self.score_belief_parity_vector, [1, -1])
            * transformed_global_inputs[
                :, self.num_global_input_features - 1 : self.num_global_input_features
            ]
        )
        sb2_offset_w = self.weight_variable(
            "sb2_offset/w",
            [1, sbv2_size],
            v1_size * 3 + 1,
            sbv2_size,
            scale_initial_weights=0.5,
        )
        sb2_offset_partial = tf.matmul(
            tf.reshape(sb2_offset_vector, [-1, 1]), sb2_offset_w
        )
        sb2_parity_w = self.weight_variable(
            "sb2_parity/w", [1, sbv2_size], v1_size * 3 + 1, sbv2_size
        )
        sb2_parity_partial = tf.matmul(
            tf.reshape(sb2_parity_vector, [-1, 1]), sb2_parity_w
        )
        sb2_layer = (
        ...
        sbscale2w = self.weight_variable(
            "sbscale2/w",
            [v1_size * 3, sbv2_size],
            v1_size * 3 + 1,
            sbv2_size,
        ...
        sbscale2b = self.weight_variable(
            "sbscale2/b",
            [sbv2_size],
            v1_size * 3 + 1,
            sbv2_size,
        ...
        sbscale2_layer = self.relu_non_spatial(
            "sbscale2/relu", tf.matmul(v1_layer_pooled, sbscale2w) + sbscale2b
        )

        sb3w = self.weight_variable(
            "sb3/w", [sbv2_size, 1], sbv2_size, 1, scale_initial_weights=0.5
        )
        sb3_layer = tf.tensordot(sb2_layer, sb3w, axes=[[2], [0]])

        sbscale3w = self.weight_variable(
            "sbscale3/w", [sbv2_size, 1], sbv2_size, 1, scale_initial_weights=0.5
        )
        ...
        scorebelief_output = tf.reshape(
            sb3_layer, [-1] + self.scorebelief_target_shape, name="scorebelief_output"
        )

        # No need for separate mask since v1_layer is already zero outside of mask bounds.
        ownership_output = (
            self.conv_only_block(
                "vownership",
                v1_layer,
                diam=1,
                in_channels=v1_num_channels,
                out_channels=1,
                scale_initial_weights=0.2,
            )
            * mask
        ...
        ownership_output = self.apply_symmetry(
            ownership_output, symmetries, inverse=True
        )
        ownership_output = tf.reshape(
            ownership_output,
            [-1] + self.ownership_target_shape,
        ...
        scoring_output = tf.reshape(
            scoring_output, [-1] + self.scoring_target_shape, name="scoring_output"
        )
        ...
        futurepos_output = self.apply_symmetry(
            futurepos_output, symmetries, inverse=True
        )
        futurepos_output = tf.reshape(
            futurepos_output,
            [-1] + self.futurepos_target_shape,
        ...
        seki_output = tf.reshape(
            seki_output, [-1] + self.seki_output_shape, name="seki_output"
        )
...
class Target_vars:
    def __init__(self, model, for_optimization, placeholders):
        ...
        td_value_prediction = tf.stack(
            [
                miscvalues_output[:, 4:7],
                miscvalues_output[:, 7:10],
                moremiscvalues_output[:, 2:5],
            ],
            axis=1,
        )
        ...
        shortterm_value_error_prediction = (
            tf.math.softplus(moremiscvalues_output[:, 0]) * 0.25
        )
        shortterm_score_error_prediction = (
            tf.math.softplus(moremiscvalues_output[:, 1]) * 30.0
        )

        # Loss function
        self.policy_target = (
            placeholders["policy_target"]
            if "policy_target" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.policy_target_shape
            )
        )
        self.policy_target1 = (
            placeholders["policy_target1"]
            if "policy_target1" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.policy_target_shape
            )
        )
        ...
        self.value_target = (
            placeholders["value_target"]
            if "value_target" in placeholders
            else tf.compat.v1.placeholder(tf.float32, [None] + model.value_target_shape)
        )
        self.td_value_target = (
            placeholders["td_value_target"]
            if "td_value_target" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.td_value_target_shape
            )
        )
        # Expected score prediction CONDITIONAL on result
        self.scoremean_target = (
            placeholders["scoremean_target"]
            if "scoremean_target" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.scoremean_target_shape
            )
        )
        self.td_score_target = (
            placeholders["td_score_target"]
            if "td_score_target" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.td_score_target_shape
            )
        )
        self.lead_target = (
            placeholders["lead_target"]
            if "lead_target" in placeholders
            else tf.compat.v1.placeholder(tf.float32, [None] + model.lead_target_shape)
        )
        # Arrival time of variance in game, unconditional
        self.variance_time_target = (
            placeholders["variance_time_target"]
            if "variance_time_target" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.variance_time_target_shape
            )
        )
        # Score belief distributions CONDITIONAL on result
        self.scorebelief_target = (
            placeholders["scorebelief_target"]
            if "scorebelief_target" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.scorebelief_target_shape
            )
        )
        # Ownership of board, CONDITIONAL on result
        self.ownership_target = (
            placeholders["ownership_target"]
            if "ownership_target" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.ownership_target_shape
            )
        )
        # Scoring of board, CONDITIONAL on result
        self.scoring_target = (
            placeholders["scoring_target"]
            if "scoring_target" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.scoring_target_shape
            )
        )
        # Future board positions, unconditional
        self.futurepos_target = (
            placeholders["futurepos_target"]
            if "futurepos_target" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.futurepos_target_shape
            )
        )
        # Seki state of final board, CONDITIONAL on result
        self.seki_target = (
            placeholders["seki_target"]
            if "seki_target" in placeholders
            else tf.compat.v1.placeholder(tf.float32, [None] + model.seki_target_shape)
        )

        self.target_weight_from_data = (
            placeholders["target_weight_from_data"]
            if "target_weight_from_data" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.target_weight_shape
            )
        )
        self.policy_target_weight = (
            placeholders["policy_target_weight"]
            if "policy_target_weight" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.policy_target_weight_shape
            )
        )
        self.policy_target_weight1 = (
            placeholders["policy_target_weight1"]
            if "policy_target_weight1" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.policy_target_weight_shape
            )
        )
        self.lead_target_weight = (
            placeholders["lead_target_weight"]
            if "lead_target_weight" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.lead_target_weight_shape
            )
        )
        self.ownership_target_weight = (
            placeholders["ownership_target_weight"]
            if "ownership_target_weight" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.ownership_target_weight_shape
            )
        )
        self.scoring_target_weight = (
            placeholders["scoring_target_weight"]
            if "scoring_target_weight" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.scoring_target_weight_shape
            )
        )
        self.futurepos_target_weight = (
            placeholders["futurepos_target_weight"]
            if "futurepos_target_weight" in placeholders
            else tf.compat.v1.placeholder(
                tf.float32, [None] + model.futurepos_target_weight_shape
            )
        )
        self.selfkomi = (
            placeholders["selfkomi"]
            if "selfkomi" in placeholders
            else tf.compat.v1.placeholder(tf.float32, [None])
        )

        model.assert_batched_shape(
            "policy_target", self.policy_target, model.policy_target_shape
        )
        model.assert_batched_shape(
            "policy_target_weight",
            self.policy_target_weight,
            model.policy_target_weight_shape,
        )
        model.assert_batched_shape(
            "policy_target1", self.policy_target1, model.policy_target_shape
        )
        model.assert_batched_shape(
            "policy_target_weight1",
            self.policy_target_weight1,
            model.policy_target_weight_shape,
        )
        model.assert_batched_shape(
            "value_target", self.value_target, model.value_target_shape
        )
        model.assert_batched_shape(
            "td_value_target", self.td_value_target, model.td_value_target_shape
        )
        model.assert_batched_shape(
            "td_score_target", self.td_score_target, model.td_score_target_shape
        )
        model.assert_batched_shape(
            "scoremean_target", self.scoremean_target, model.scoremean_target_shape
        )
        model.assert_batched_shape(
            "lead_target", self.lead_target, model.lead_target_shape
        )
        model.assert_batched_shape(
            "variance_time_target",
            self.variance_time_target,
            model.variance_time_target_shape,
        )
        model.assert_batched_shape(
            "scorebelief_target",
            self.scorebelief_target,
            model.scorebelief_target_shape,
        )
        model.assert_batched_shape(
            "ownership_target", self.ownership_target, model.ownership_target_shape
        )
        model.assert_batched_shape(
            "scoring_target", self.scoring_target, model.scoring_target_shape
        )
        model.assert_batched_shape(
            "futurepos_target", self.futurepos_target, model.futurepos_target_shape
        )
        model.assert_batched_shape(
            "seki_target", self.seki_target, model.seki_target_shape
        )
        model.assert_batched_shape(
            "target_weight_from_data",
            self.target_weight_from_data,
            model.target_weight_shape,
        )
        model.assert_batched_shape(
            "lead_target_weight",
            self.lead_target_weight,
            model.lead_target_weight_shape,
        )
        model.assert_batched_shape(
            "ownership_target_weight",
            self.ownership_target_weight,
            model.ownership_target_weight_shape,
        )
        model.assert_batched_shape(
            "scoring_target_weight",
            self.scoring_target_weight,
            model.scoring_target_weight_shape,
        )
        model.assert_batched_shape(
            "futurepos_target_weight",
            self.futurepos_target_weight,
            model.futurepos_target_weight_shape,
        )
        ...
        self.policy_loss_unreduced = self.policy_target_weight * (
            tf.nn.softmax_cross_entropy_with_logits_v2(
                labels=self.policy_target, logits=policy_output[:, :, 0]
            )
        )
        self.policy1_loss_unreduced = (
            self.policy_target_weight1
            * 0.15
            * (
                tf.nn.softmax_cross_entropy_with_logits_v2(
                    labels=self.policy_target1, logits=policy_output[:, :, 1]
                )
            )
        )

        self.value_loss_unreduced = 1.20 * tf.nn.softmax_cross_entropy_with_logits_v2(
            labels=self.value_target, logits=value_output
        )

        self.td_value_loss_unreduced = tf.constant(
            [0.55, 0.55, 0.15], dtype=tf.float32
        ) * (
            tf.nn.softmax_cross_entropy_with_logits_v2(
                labels=self.td_value_target, logits=td_value_prediction
            )
            -
        ...
        self.td_value_loss_unreduced = tf.reduce_sum(
            self.td_value_loss_unreduced, axis=1
        )

        self.td_score_loss_unreduced = (
            0.0004
            * self.ownership_target_weight
            * (
                tf.reduce_sum(
                    huber_loss(self.td_score_target, td_score_prediction, delta=12.0),
                    axis=1,
                )
            )
        )

        self.scorebelief_cdf_loss_unreduced = (
            0.020
            * self.ownership_target_weight
            * (
                tf.reduce_sum(
                    tf.square(
                        tf.cumsum(self.scorebelief_target, axis=1)
                        - tf.cumsum(tf.nn.softmax(scorebelief_output, axis=1), axis=1)
                    ),
                    axis=1,
                )
            )
        )
        self.scorebelief_pdf_loss_unreduced = (
            0.020
            * self.ownership_target_weight
            * (
                tf.nn.softmax_cross_entropy_with_logits_v2(
                    labels=self.scorebelief_target, logits=scorebelief_output
                )
            )
        )
        ...
        self.ownership_loss_unreduced = (
            ...
            * (
                tf.reduce_sum(
                    ...
                    * tf.reshape(
                        model.mask_before_symmetry, [-1, model.pos_len, model.pos_len]
                    ),
                    axis=[1, 2],
                )
                / model.mask_sum_hw
        ...
        self.scoring_loss_unreduced = (
            1.0
            * self.scoring_target_weight
            * (
                tf.reduce_sum(
                    tf.square(self.scoring_target - scoring_output)
                    * tf.reshape(
                        model.mask_before_symmetry, [-1, model.pos_len, model.pos_len]
                    ),
                    axis=[1, 2],
        ...
        self.scoring_loss_unreduced = 4.0 * (
            tf.sqrt(self.scoring_loss_unreduced * 0.5 + 1.0) - 1.0
        )
        ...
        self.futurepos_loss_unreduced = (
            ...
            * (
                tf.reduce_sum(
                    ...
                    * tf.reshape(
                        model.mask_before_symmetry,
                        [-1, model.pos_len, model.pos_len, 1],
                    )
                    * tf.reshape(
                        tf.constant([1, 0.25], dtype=tf.float32), [1, 1, 1, 2]
                    ),
                    axis=[1, 2, 3],
                )
        ...
        unowned_proportion = tf.reduce_sum(
            unowned_target
            * tf.reshape(
                model.mask_before_symmetry, [-1, model.pos_len, model.pos_len]
            ),
        ...
        unowned_proportion = tf.reduce_mean(
            unowned_proportion * self.ownership_target_weight
        )
        if model.is_training:
            moving_unowned_proportion = tf.compat.v1.get_variable(
                initializer=1.0, name="moving_unowned_proportion", trainable=False
            )
            moving_unowned_op = tf.keras.backend.moving_average_update(
                moving_unowned_proportion, unowned_proportion, 0.998
            )
        ...
        self.seki_loss_unreduced = (
            tf.reduce_sum(
                tf.nn.softmax_cross_entropy_with_logits_v2(
                    labels=tf.stack(
                        [
                            1.0 - tf.square(self.seki_target),
                            tf.nn.relu(self.seki_target),
                            tf.nn.relu(-self.seki_target),
                        ],
                        axis=3,
                    ),
        ...
        self.seki_loss_unreduced = self.seki_loss_unreduced + 0.5 * (
            tf.reduce_sum(
                tf.nn.softmax_cross_entropy_with_logits_v2(
                    ...
                    logits=tf.stack(
                        [seki_output[:, :, :, 3], tf.zeros_like(self.ownership_target)],
                        axis=3,
                    ),
                )
                * tf.reshape(
        ...
        self.seki_loss_unreduced = (
            seki_weight_scale * self.ownership_target_weight * self.seki_loss_unreduced
        )
        self.seki_weight_scale = seki_weight_scale

        # This is conditional upon there being a result
        expected_score_from_belief = tf.reduce_sum(
            scorebelief_probs * model.score_belief_offset_vector, axis=1
        )
        ...
        self.scoremean_loss_unreduced = (
            0.0015
            * self.ownership_target_weight
            * huber_loss(self.scoremean_target, scoremean_prediction, delta=12.0)
        )
        self.lead_loss_unreduced = (
            0.0060
            * self.lead_target_weight
            * huber_loss(self.lead_target, lead_prediction, delta=8.0)
        )
        self.variance_time_loss_unreduced = (
            0.0003
            * self.ownership_target_weight
            * huber_loss(
                self.variance_time_target, variance_time_prediction, delta=50.0
            )
        )

        stdev_of_belief = tf.sqrt(
            0.001
            + tf.reduce_sum(
                scorebelief_probs
                * tf.square(
                    tf.reshape(model.score_belief_offset_vector, [1, -1])
                    - tf.reshape(expected_score_from_belief, [-1, 1])
                ),
                axis=1,
            )
        )
        beliefstdevdiff = stdev_of_belief - scorestdev_prediction
        self.scorestdev_reg_loss_unreduced = 0.004 * huber_loss(
            stdev_of_belief, scorestdev_prediction, delta=10.0
        )
        ...
        self.shortterm_value_error_loss_unreduced = (
            2.0
            * self.ownership_target_weight
            * huber_loss(
                shorttermdiffvaluesq, shortterm_value_error_prediction, delta=0.4
            )
        )
        # Use self.ownership_target_weight to make sure that we only have this target when we genuinely played out the game
        self.shortterm_score_error_loss_unreduced = (
            0.00002
            * self.ownership_target_weight
            * huber_loss(
                shorttermdiffscoresq, shortterm_score_error_prediction, delta=100.0
            )
        )
        ...
        if (
            "use_fixed_sbscaling" in model.config
            and model.config["use_fixed_sbscaling"]
        ):
            self.scale_reg_loss_unreduced = tf.reshape(
                0.0002
                * tf.add_n(
                    [tf.square(variable) for variable in model.prescale_variables]
                ),
                [-1],
            )
        else:
        ...
        self.policy_loss = tf.reduce_sum(
            self.target_weight_used * self.policy_loss_unreduced,
            name="losses/policy_loss",
        )
        self.policy1_loss = tf.reduce_sum(
            self.target_weight_used * self.policy1_loss_unreduced,
            name="losses/policy1_loss",
        )
        self.value_loss = tf.reduce_sum(
            self.target_weight_used * self.value_loss_unreduced,
            name="losses/value_loss",
        )
        self.td_value_loss = tf.reduce_sum(
            self.target_weight_used * self.td_value_loss_unreduced,
            name="losses/td_value_loss",
        )
        self.td_score_loss = tf.reduce_sum(
            self.target_weight_used * self.td_score_loss_unreduced,
            name="losses/td_score_loss",
        )
        self.scoremean_loss = tf.reduce_sum(
            self.target_weight_used * self.scoremean_loss_unreduced,
            name="losses/scoremean_loss",
        )
        self.lead_loss = tf.reduce_sum(
            self.target_weight_used * self.lead_loss_unreduced, name="losses/lead_loss"
        )
        self.variance_time_loss = tf.reduce_sum(
            self.target_weight_used * self.variance_time_loss_unreduced,
            name="losses/variance_time_loss",
        )
        self.scorebelief_pdf_loss = tf.reduce_sum(
            self.target_weight_used * self.scorebelief_pdf_loss_unreduced,
            name="losses/scorebelief_pdf_loss",
        )
        self.scorebelief_cdf_loss = tf.reduce_sum(
            self.target_weight_used * self.scorebelief_cdf_loss_unreduced,
            name="losses/scorebelief_cdf_loss",
        )
        self.ownership_loss = tf.reduce_sum(
            self.target_weight_used * self.ownership_loss_unreduced,
            name="losses/ownership_loss",
        )
        self.scoring_loss = tf.reduce_sum(
            self.target_weight_used * self.scoring_loss_unreduced,
            name="losses/scoring_loss",
        )
        self.futurepos_loss = tf.reduce_sum(
            self.target_weight_used * self.futurepos_loss_unreduced,
            name="losses/futurepos_loss",
        )
        self.seki_loss = tf.reduce_sum(
            self.target_weight_used * self.seki_loss_unreduced, name="losses/seki_loss"
        )
        self.scorestdev_reg_loss = tf.reduce_sum(
            self.target_weight_used * self.scorestdev_reg_loss_unreduced,
            name="losses/scorestdev_reg_loss",
        )
        self.shortterm_value_error_loss = tf.reduce_sum(
            self.target_weight_used * self.shortterm_value_error_loss_unreduced,
            name="losses/sloss",
        )
        self.shortterm_score_error_loss = tf.reduce_sum(
            self.target_weight_used * self.shortterm_score_error_loss_unreduced,
        ...
        self.scale_reg_loss = tf.reduce_sum(
            self.target_weight_used * self.scale_reg_loss_unreduced,
            name="losses/scale_reg_loss",
        )

        self.weight_sum = tf.reduce_sum(
            self.target_weight_used, name="losses/weight_sum"
        )

        if for_optimization:
            # Prior/Regularization
            self.l2_reg_coeff = (
                placeholders["l2_reg_coeff"]
                if "l2_reg_coeff" in placeholders
                else tf.compat.v1.placeholder(tf.float32)
            )
            self.reg_loss_per_weight = self.l2_reg_coeff * (
                tf.add_n([tf.nn.l2_loss(variable) for variable in model.reg_variables])
                + 0.05
                * tf.add_n(
                    [tf.nn.l2_loss(variable) for variable in model.reg_variables_tiny]
                )
            )
...
class Metrics:
    def __init__(self, model, target_vars, include_debug_stats):
        ...
        self.top1_prediction = tf.equal(
            tf.argmax(model.policy_output[:, :, 0], 1), policy_target_idxs
        )
        self.top4_prediction = tf.nn.in_top_k(
            model.policy_output[:, :, 0], policy_target_idxs, 4
        )
        ...
        self.value_entropy_unreduced = tf.nn.softmax_cross_entropy_with_logits_v2(
            labels=tf.nn.softmax(model.value_output, axis=1), logits=model.value_output
        )
        self.value_conf_unreduced = 4 * tf.square(
            tf.nn.sigmoid(model.value_output[:, 0] - model.value_output[:, 1]) - 0.5
        )
        self.policy_target_entropy_unreduced = target_vars.policy_target_weight * (
            -tf.reduce_sum(
                target_vars.policy_target
                * tf.math.log(target_vars.policy_target + (1e-20)),
                axis=1,
            )
        )
        self.accuracy1 = tf.reduce_sum(
            target_vars.target_weight_used * self.accuracy1_unreduced,
            name="metrics/accuracy1",
        )
        self.accuracy4 = tf.reduce_sum(
            target_vars.target_weight_used * self.accuracy4_unreduced,
            name="metrics/accuracy4",
        )
        self.value_entropy = tf.reduce_sum(
            target_vars.target_weight_used * self.value_entropy_unreduced,
            name="metrics/value_entropy",
        )
        self.value_conf = tf.reduce_sum(
            target_vars.target_weight_used * self.value_conf_unreduced,
            name="metrics/value_conf",
        )
        self.policy_target_entropy = tf.reduce_sum(
            target_vars.target_weight_used * self.policy_target_entropy_unreduced,
        ...
        if include_debug_stats:
            ...
            def reduce_norm(x, axis=None, keepdims=False):
                return tf.sqrt(
                    tf.reduce_mean(tf.square(x), axis=axis, keepdims=keepdims)
                )
            ...
            def reduce_stdev(x, axis=None, keepdims=False):
                ...
                return tf.sqrt(
                    tf.reduce_mean(devs_squared, axis=axis, keepdims=keepdims)
                )
            ...
            self.activated_prop_by_layer = dict(
                [
                    (
                        name,
                        tf.reduce_mean(
                            tf.count_nonzero(layer, axis=[1, 2])
                            / layer.shape[1].value
                            / layer.shape[2].value,
                            axis=0,
                        ),
                    )
                    for (name, layer) in model.outputs_by_layer
                ]
            )
            self.mean_output_by_layer = dict(
                [
                    (name, tf.reduce_mean(layer, axis=[0, 1, 2]))
                    for (name, layer) in model.outputs_by_layer
                ]
            )
            self.stdev_output_by_layer = dict(
                [
                    (name, reduce_stdev(layer, axis=[0, 1, 2]))
                    for (name, layer) in model.outputs_by_layer
                ]
            )
            self.mean_weights_by_var = dict(
                [
                    (v.name, tf.reduce_mean(v))
                    for v in tf.compat.v1.trainable_variables()
                ]
            )
...
class ModelUtils:
    ...
    @staticmethod
    def build_model_from_tfrecords_features(
        features,
        mode,
        print_model,
        trainlog,
        ...
        # Unpack binary data
        bitmasks = tf.reshape(
            tf.constant([128, 64, 32, 16, 8, 4, 2, 1], dtype=tf.uint8), [1, 1, 1, 8]
        )
        binchw = tf.reshape(
            tf.bitwise.bitwise_and(tf.expand_dims(binchwp, axis=3), bitmasks),
            [-1, num_bin_input_features, ((pos_len * pos_len + 7) // 8) * 8],
        )
        ...
        placeholders["symmetries"] = tf.greater(
            tf.random.uniform([3], minval=0, maxval=2, dtype=tf.int32),
        ...
        policy_target0 = policy_target0 / tf.reduce_sum(
            policy_target0, axis=1, keepdims=True
        )
        ...
        policy_target1 = policy_target1 / tf.reduce_sum(
            policy_target1, axis=1, keepdims=True
        )
        ...
        placeholders["td_value_target"] = tf.stack(
            [
                features["gtnc"][:, 4:7],
                features["gtnc"][:, 8:11],
                features["gtnc"][:, 12:15],
        ...
        placeholders["td_score_target"] = tf.stack(
            [features["gtnc"][:, 7], features["gtnc"][:, 11], features["gtnc"][:, 15]],
            axis=1,
        )
        ...
        placeholders["futurepos_target"] = tf.transpose(
            features["vtnchw"][:, 2:4], [0, 2, 3, 1]
        )
        ...
        if mode == tf.estimator.ModeKeys.EVAL:
            ...
            target_vars = Target_vars(
                model, for_optimization=True, placeholders=placeholders
            )
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            global_step_samples = global_step_float * tf.constant(
                batch_size, dtype=tf.float32
            )

            lr_base = (0.00003 if model.use_fixup else 0.00006) * (
                1.0 if lr_scale is None else lr_scale
            )
            per_sample_learning_rate = tf.constant(
                lr_base
            ) * tf.compat.v1.train.piecewise_constant(
                global_step_samples, boundaries=[5.0e6], values=[1.0 / 3.0, 1.0]
            )
            ...
            with tf.control_dependencies(update_ops):
                optimizer = tf.compat.v1.train.MomentumOptimizer(
                    per_sample_learning_rate, momentum=0.9, use_nesterov=True
                )
                ...
                for grad, x in gradients:
                    ...
                    if num_gpus_used > 1:
                        adjusted_gradients.append(
                            (
                                adjusted_grad
                                * tf.constant(num_gpus_used, dtype=tf.float32),
                                x,
                            )
                        )
                    else:
                ...
                gnorm_cap = (
                    (2500.0 if model.use_fixup else 4000.0)
                    / math.sqrt(num_gpus_used)
                    * (1.0 if gnorm_clip_scale is None else gnorm_clip_scale)
                )
                # Loosen gradient clipping as we shift to smaller learning rates
                gnorm_cap = gnorm_cap / math.sqrt(
                    1.0 if lr_scale is None else max(0.0000001, lr_scale)
                )
                (adjusted_gradients_clipped, gnorm) = tf.clip_by_global_norm(
                    [x[0] for x in adjusted_gradients], gnorm_cap
                )
                adjusted_gradients_clipped = list(
                    zip(adjusted_gradients_clipped, [x[1] for x in adjusted_gradients])
                )
                metrics.gnorm = gnorm
                metrics.excess_gnorm = tf.nn.relu(gnorm - gnorm_cap)
                train_step = optimizer.apply_gradients(
                    adjusted_gradients_clipped, global_step=global_step
                )

            if print_model:
                trainlog("Model version: %d" % model.version)
                ModelUtils.print_trainable_variables(trainlog)
                for update_op in tf.compat.v1.get_collection(
                    tf.compat.v1.GraphKeys.UPDATE_OPS
                ):
                    trainlog("Additional update op on train step: %s" % update_op.name)
                trainlog(
                    "Supporting japanese rules: " + str(model.support_japanese_rules)
                )

...
class Model:
    ...
    def __init__(self, config, pos_len, placeholders, is_training=False):
        ...
        if "support_japanese_rules" in config and not config["support_japanese_rules"]:
            self.support_japanese_rules = False

        self.use_scoremean_as_lead = False  # by default
        if "use_scoremean_as_lead" in config and config["use_scoremean_as_lead"]:
            self.use_scoremean_as_lead = True

        self.build_model(config, placeholders)

    def assert_batched_shape(self, name, tensor, shape):
        if len(tensor.shape) != len(shape) + 1 or [int(tensor.shape[i + 1].value) for i in range(len(shape))] != [
            int(x) for x in shape
        ]:
            raise ValueError(
                f"{name} should have shape {str(shape)} after a batch dimension but instead it had shape"
                f" {[str(x.value) for x in tensor.shape]}"
            )

    def assert_shape(self, name, tensor, shape):
        if len(tensor.shape) != len(shape) or [int(x.value) for x in tensor.shape] != [int(x) for x in shape]:
            raise ValueError(
                f"{name} should have shape {str(shape)} but instead it had shape {[str(x.value) for x in tensor.shape]}"
            )
    ...
    def batchnorm_and_mask(self, name, tensor, mask, mask_sum, use_gamma_in_fixup=False):
        if self.use_fixup:
            ...
            if use_gamma_in_fixup:
                gamma = self.weight_variable_init_constant(f"{name}/gamma", [tensor.shape[3].value], 1.0)
                beta = self.weight_variable_init_constant(f"{name}/beta", [tensor.shape[3].value], 0.0, reg="tiny")
        ...
        moving_mean = tf.compat.v1.get_variable(
            ...
            name=f"{name}/moving_mean",
            trainable=False,
            collections=collections,
        )
        moving_var = tf.compat.v1.get_variable(
            initializer=tf.ones([num_channels]),
            name=f"{name}/moving_variance",
            trainable=False,
            collections=collections,
        )
        beta = self.weight_variable_init_constant(f"{name}/beta", [tensor.shape[3].value], 0.0, reg=False)
    ...
    def weight_variable(
        self, name, shape, num_inputs, num_outputs, scale_initial_weights=1.0, extra_initial_weight=None, reg=True
    ):
    ...
    def apply_symmetry(self, tensor, symmetries, inverse):
        ...
        tensor = tf.cond(transp, lambda: tf.transpose(tensor, [0, 2, 1, 3]), lambda: tensor)
    ...
    def conv_block(self, name, in_layer, mask, mask_sum, diam, in_channels, out_channels, scale_initial_weights=1.0):
        weights = self.conv_weight_variable(f"{name}/w", diam, diam, in_channels, out_channels, scale_initial_weights)
        convolved = self.conv2d(in_layer, weights)
        self.outputs_by_layer.append((f"{name}/prenorm", convolved))
        out_layer = self.relu(
            f"{name}/relu",
            self.batchnorm_and_mask(f"{name}/norm", convolved, mask, mask_sum),
        )
        self.outputs_by_layer.append((name, out_layer))
        return out_layer

    # Convolution only, no batch norm or nonlinearity
    def conv_only_block(self, name, in_layer, diam, in_channels, out_channels, scale_initial_weights=1.0, reg=True):
    ...
    def res_conv_block(
        self, name, in_layer, mask, mask_sum, diam, main_channels, mid_channels, scale_initial_weights=1.0
    ):
        trans1_layer = self.relu(
            f"{name}/relu1",
            self.batchnorm_and_mask(f"{name}/norm1", in_layer, mask, mask_sum),
        )
        self.outputs_by_layer.append((f"{name}/trans1", trans1_layer))
        ...
        self.outputs_by_layer.append((f"{name}/conv1", conv1_layer))

        trans2_layer = self.relu(
            f"{name}/relu2",
            self.batchnorm_and_mask(f"{name}/norm2", conv1_layer, mask, mask_sum, use_gamma_in_fixup=True),
        )
        self.outputs_by_layer.append((f"{name}/trans2", trans2_layer))
        ...
        self.outputs_by_layer.append((f"{name}/conv2", conv2_layer))
    ...
    def global_res_conv_block(
        ...
        self.outputs_by_layer.append((f"{name}/conv1a", conv1a_layer))
        self.outputs_by_layer.append((f"{name}/conv1b", conv1b_layer))

        trans1b_layer = self.relu(
            f"{name}/trans1b",
            self.batchnorm_and_mask(f"{name}/norm1b", conv1b_layer, mask, mask_sum),
        )
    ...
    def build_model(self, config, placeholders):
        ...
        bin_inputs = (
            ...
            else tf.compat.v1.placeholder(tf.float32, [None] + self.bin_input_shape, name="bin_inputs")
        )
        global_inputs = (
            placeholders["global_inputs"]
            if "global_inputs" in placeholders
            else tf.compat.v1.placeholder(tf.float32, [None] + self.global_input_shape, name="global_inputs")
        )
        symmetries = (
        ...
        include_history = (
            ...
            else tf.compat.v1.placeholder(tf.float32, [None] + [5], name="include_history")
        )
        ...
        hist_matrix_base = tf.reshape(
            tf.constant(hist_matrix_base), [1, self.num_bin_input_features, self.num_bin_input_features]
        )
        ...
        transformed_global_inputs = global_inputs * tf.pad(
            include_history,
            [(0, 0), (0, self.num_global_input_features - include_history.shape[1].value)],
            constant_values=1.0,
        )
        ...
        if "use_initial_conv_3" in config and config["use_initial_conv_3"]:
            trunk = self.conv_only_block(
                "conv1", cur_layer, diam=3, in_channels=input_num_channels, out_channels=trunk_num_channels
            )
            self.initial_conv = ("conv1", 3, input_num_channels, trunk_num_channels)
        else:
            trunk = self.conv_only_block(
                "conv1", cur_layer, diam=5, in_channels=input_num_channels, out_channels=trunk_num_channels
            )
        ...
        for i in range(len(block_kind)):
            ...
            if kind == "regular":
                ...
                trunk = self.merge_residual(name, trunk, residual)
                self.blocks.append(
                    ("dilated_block", name, 3, trunk_num_channels, regular_num_channels, dilated_num_channels, 3)
                )
            elif kind == "gpool":
                ...
                trunk = self.merge_residual(name, trunk, residual)
                self.blocks.append(
                    ("gpool_block", name, 3, trunk_num_channels, regular_num_channels, gpool_num_channels)
                )
            else:
                assert False
        ...
        p1_intermediate_conv = self.conv_only_block(
            "p1/intermediate_conv", p0_layer, diam=1, in_channels=trunk_num_channels, out_channels=p1_num_channels
        )
        ...
        g1_layer = self.conv_block(
            "g1", p0_layer, mask, mask_sum, diam=1, in_channels=trunk_num_channels, out_channels=g1_num_channels
        )
        ...
        matmulg2w = self.weight_variable(
            "matmulg2w", [g2_num_channels, p1_num_channels], g2_num_channels * 4, p1_num_channels
        )
        ...
        p2_layer = self.conv_only_block(
            "p2", p1_layer, diam=1, in_channels=p1_num_channels, out_channels=2, scale_initial_weights=0.3
        )
        p2_layer = (
        ...
        matmulpass = self.weight_variable(
            "matmulpass", [g2_num_channels, 2], g2_num_channels * 8, 2, scale_initial_weights=0.3
        )
        ...
        v1_layer = self.conv_block(
            "v1", v0_layer, mask, mask_sum, diam=1, in_channels=trunk_num_channels, out_channels=v1_num_channels
        )
        ...
        moremiscvalues_output = tf.reshape(
            mmv3_layer, [-1] + self.moremiscvalues_target_shape, name="moremiscvalues_output"
        )
        ...
        self.score_belief_offset_vector = np.array(
            [float(i - scorebelief_mid) + 0.5 for i in range(scorebelief_len)], dtype=np.float32
        )
        self.score_belief_parity_vector = np.array(
            [0.5 - float((i - scorebelief_mid) % 2) for i in range(scorebelief_len)], dtype=np.float32
        )
        ...
        sb2b = self.weight_variable(
            "sb2/b", [sbv2_size], v1_size * 3 + 1, sbv2_size, scale_initial_weights=0.2, reg="tiny"
        )
        ...
        sb2_parity_vector = (
            ...
            * transformed_global_inputs[:, self.num_global_input_features - 1 : self.num_global_input_features]
        )
        sb2_offset_w = self.weight_variable(
            "sb2_offset/w", [1, sbv2_size], v1_size * 3 + 1, sbv2_size, scale_initial_weights=0.5
        )
        ...
        sbscale2w = self.weight_variable(
            "sbscale2/w", [v1_size * 3, sbv2_size], v1_size * 3 + 1, sbv2_size, scale_initial_weights=0.5
        )
        sbscale2b = self.weight_variable(
            "sbscale2/b", [sbv2_size], v1_size * 3 + 1, sbv2_size, scale_initial_weights=0.2, reg="tiny"
        )
        ...
        ownership_output = (
            self.conv_only_block(
                "vownership", v1_layer, diam=1, in_channels=v1_num_channels, out_channels=1, scale_initial_weights=0.2
            )
            * mask
        )
        ...
        scoring_output = (
            self.conv_only_block(
                "vscoring", v1_layer, diam=1, in_channels=v1_num_channels, out_channels=1, scale_initial_weights=0.2
            )
            * mask
        )
        ...
        futurepos_output = (
            self.conv_only_block(
                "futurepos", v0_layer, diam=1, in_channels=trunk_num_channels, out_channels=2, scale_initial_weights=0.2
            )
            * mask
        )
        ...
        seki_output = (
            self.conv_only_block(
                "seki", v0_layer, diam=1, in_channels=trunk_num_channels, out_channels=4, scale_initial_weights=0.2
            )
            * mask
        )
...
class Target_vars:
    def __init__(self, model, for_optimization, placeholders):
        ...
        td_value_prediction = tf.stack(
            [miscvalues_output[:, 4:7], miscvalues_output[:, 7:10], moremiscvalues_output[:, 2:5]], axis=1
        )
        ...
        self.policy_target = (
            ...
            else tf.compat.v1.placeholder(tf.float32, [None] + model.policy_target_shape)
        )
        self.policy_target1 = (
        ...
        self.td_value_target = (
            ...
            else tf.compat.v1.placeholder(tf.float32, [None] + model.td_value_target_shape)
        )
        ...
        self.scoremean_target = (
            ...
            else tf.compat.v1.placeholder(tf.float32, [None] + model.scoremean_target_shape)
        )
        self.td_score_target = (
            placeholders["td_score_target"]
            if "td_score_target" in placeholders
            else tf.compat.v1.placeholder(tf.float32, [None] + model.td_score_target_shape)
        )
        self.lead_target = (
        ...
        self.variance_time_target = (
            ...
            else tf.compat.v1.placeholder(tf.float32, [None] + model.variance_time_target_shape)
        )
        ...
        self.scorebelief_target = (
            ...
            else tf.compat.v1.placeholder(tf.float32, [None] + model.scorebelief_target_shape)
        )
        ...
        self.ownership_target = (
            ...
            else tf.compat.v1.placeholder(tf.float32, [None] + model.ownership_target_shape)
        )
        # Scoring of board, CONDITIONAL on result
        self.scoring_target = (
            placeholders["scoring_target"]
            if "scoring_target" in placeholders
            else tf.compat.v1.placeholder(tf.float32, [None] + model.scoring_target_shape)
        )
        ...
        self.futurepos_target = (
            ...
            else tf.compat.v1.placeholder(tf.float32, [None] + model.futurepos_target_shape)
        )
        ...
        self.target_weight_from_data = (
            ...
            else tf.compat.v1.placeholder(tf.float32, [None] + model.target_weight_shape)
        )
        self.policy_target_weight = (
            placeholders["policy_target_weight"]
            if "policy_target_weight" in placeholders
            else tf.compat.v1.placeholder(tf.float32, [None] + model.policy_target_weight_shape)
        )
        ...
        self.lead_target_weight = (
            ...
            else tf.compat.v1.placeholder(tf.float32, [None] + model.lead_target_weight_shape)
        )
        self.ownership_target_weight = (
            placeholders["ownership_target_weight"]
            if "ownership_target_weight" in placeholders
            else tf.compat.v1.placeholder(tf.float32, [None] + model.ownership_target_weight_shape)
        )
        self.scoring_target_weight = (
            placeholders["scoring_target_weight"]
            if "scoring_target_weight" in placeholders
            else tf.compat.v1.placeholder(tf.float32, [None] + model.scoring_target_weight_shape)
        )
        self.futurepos_target_weight = (
            placeholders["futurepos_target_weight"]
            if "futurepos_target_weight" in placeholders
            else tf.compat.v1.placeholder(tf.float32, [None] + model.futurepos_target_weight_shape)
        )
        self.selfkomi = (
            placeholders["selfkomi"] if "selfkomi" in placeholders else tf.compat.v1.placeholder(tf.float32, [None])
        )
        ...
        model.assert_batched_shape(
            "policy_target_weight1", self.policy_target_weight1, model.policy_target_weight_shape
        )
        ...
        model.assert_batched_shape(
            "ownership_target_weight", self.ownership_target_weight, model.ownership_target_weight_shape
        )
        model.assert_batched_shape(
            "scoring_target_weight", self.scoring_target_weight, model.scoring_target_weight_shape
        )
        model.assert_batched_shape(
            "futurepos_target_weight", self.futurepos_target_weight, model.futurepos_target_weight_shape
        )
        ...
        self.td_value_loss_unreduced = tf.constant([0.55, 0.55, 0.15], dtype=tf.float32) * (
            tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.td_value_target, logits=td_value_prediction)
            -
            # Subtract out the entropy, so as to get loss 0 at perfect prediction
            tf.nn.softmax_cross_entropy_with_logits_v2(
                labels=self.td_value_target, logits=tf.math.log(self.td_value_target + 1.0e-30)
            )
        )
        ...
        self.scorebelief_pdf_loss_unreduced = (
            0.020
            * self.ownership_target_weight
            * (tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.scorebelief_target, logits=scorebelief_output))
        )
        ...
        unowned_proportion = tf.reduce_sum(
            unowned_target * tf.reshape(model.mask_before_symmetry, [-1, model.pos_len, model.pos_len]), axis=[1, 2]
        ) / (
        ...
        self.scoremean_loss_unreduced = (
            0.0015 * self.ownership_target_weight * huber_loss(self.scoremean_target, scoremean_prediction, delta=12.0)
        )
        self.lead_loss_unreduced = (
            0.0060 * self.lead_target_weight * huber_loss(self.lead_target, lead_prediction, delta=8.0)
        )
        self.variance_time_loss_unreduced = (
            0.0003
            * self.ownership_target_weight
            * huber_loss(self.variance_time_target, variance_time_prediction, delta=50.0)
        )
        ...
        self.shortterm_value_error_loss_unreduced = (
            2.0
            * self.ownership_target_weight
            * huber_loss(shorttermdiffvaluesq, shortterm_value_error_prediction, delta=0.4)
        )
        # Use self.ownership_target_weight to make sure that we only have this target when we genuinely played out the game
        self.shortterm_score_error_loss_unreduced = (
            0.00002
            * self.ownership_target_weight
            * huber_loss(shorttermdiffscoresq, shortterm_score_error_prediction, delta=100.0)
        )
        ...
        if "use_fixed_sbscaling" in model.config and model.config["use_fixed_sbscaling"]:
            self.scale_reg_loss_unreduced = tf.reshape(
                0.0002 * tf.add_n([tf.square(variable) for variable in model.prescale_variables]), [-1]
            )
        else:
            self.scale_reg_loss_unreduced = tf.reshape(
                0.0004 * tf.add_n([tf.square(variable) for variable in model.prescale_variables]), [-1]
            )
        # self.scale_reg_loss_unreduced = tf.zeros_like(self.winloss_reg_loss_unreduced)

        self.policy_loss = tf.reduce_sum(
            self.target_weight_used * self.policy_loss_unreduced, name="losses/policy_loss"
        )
        self.policy1_loss = tf.reduce_sum(
            self.target_weight_used * self.policy1_loss_unreduced, name="losses/policy1_loss"
        )
        ...
        self.td_value_loss = tf.reduce_sum(
            self.target_weight_used * self.td_value_loss_unreduced, name="losses/td_value_loss"
        )
        self.td_score_loss = tf.reduce_sum(
            self.target_weight_used * self.td_score_loss_unreduced, name="losses/td_score_loss"
        )
        self.scoremean_loss = tf.reduce_sum(
            self.target_weight_used * self.scoremean_loss_unreduced, name="losses/scoremean_loss"
        )
        ...
        self.variance_time_loss = tf.reduce_sum(
            self.target_weight_used * self.variance_time_loss_unreduced, name="losses/variance_time_loss"
        )
        self.scorebelief_pdf_loss = tf.reduce_sum(
            self.target_weight_used * self.scorebelief_pdf_loss_unreduced, name="losses/scorebelief_pdf_loss"
        )
        self.scorebelief_cdf_loss = tf.reduce_sum(
            self.target_weight_used * self.scorebelief_cdf_loss_unreduced, name="losses/scorebelief_cdf_loss"
        )
        self.ownership_loss = tf.reduce_sum(
            self.target_weight_used * self.ownership_loss_unreduced, name="losses/ownership_loss"
        )
        self.scoring_loss = tf.reduce_sum(
            self.target_weight_used * self.scoring_loss_unreduced, name="losses/scoring_loss"
        )
        self.futurepos_loss = tf.reduce_sum(
            self.target_weight_used * self.futurepos_loss_unreduced, name="losses/futurepos_loss"
        )
        ...
        self.scorestdev_reg_loss = tf.reduce_sum(
            self.target_weight_used * self.scorestdev_reg_loss_unreduced, name="losses/scorestdev_reg_loss"
        )
        self.shortterm_value_error_loss = tf.reduce_sum(
            self.target_weight_used * self.shortterm_value_error_loss_unreduced, name="losses/sloss"
        )
        ...
        self.scale_reg_loss = tf.reduce_sum(
            self.target_weight_used * self.scale_reg_loss_unreduced, name="losses/scale_reg_loss"
        )
        ...
        if for_optimization:
            ...
            self.l2_reg_coeff = (
                placeholders["l2_reg_coeff"] if "l2_reg_coeff" in placeholders else tf.compat.v1.placeholder(tf.float32)
            )
...
class Metrics:
    def __init__(self, model, target_vars, include_debug_stats):
        ...
        self.accuracy1 = tf.reduce_sum(
            target_vars.target_weight_used * self.accuracy1_unreduced, name="metrics/accuracy1"
        )
        self.accuracy4 = tf.reduce_sum(
            target_vars.target_weight_used * self.accuracy4_unreduced, name="metrics/accuracy4"
        )
        self.value_entropy = tf.reduce_sum(
            target_vars.target_weight_used * self.value_entropy_unreduced, name="metrics/value_entropy"
        )
        self.value_conf = tf.reduce_sum(
            target_vars.target_weight_used * self.value_conf_unreduced, name="metrics/value_conf"
        )
        self.policy_target_entropy = tf.reduce_sum(
            target_vars.target_weight_used * self.policy_target_entropy_unreduced, name="metrics/policy_target_entropy"
        )
        ...
        if include_debug_stats:
            ...
            self.activated_prop_by_layer = dict(
                [
                    (
                        name,
                        tf.reduce_mean(
                            tf.count_nonzero(layer, axis=[1, 2]) / layer.shape[1].value / layer.shape[2].value, axis=0
                        ),
                    )
            ...
            self.mean_weights_by_var = dict([(v.name, tf.reduce_mean(v)) for v in tf.compat.v1.trainable_variables()])
            self.norm_weights_by_var = dict([(v.name, reduce_norm(v)) for v in tf.compat.v1.trainable_variables()])
...
class ModelUtils:
    ...
    def build_model_from_tfrecords_features(
        ...
        placeholders = {
            "bin_inputs": binhwc,
            "global_inputs": features["ginc"],
            "symmetries": tf.greater(
        ...
        placeholders["td_value_target"] = tf.stack(
            [features["gtnc"][:, 4:7], features["gtnc"][:, 8:11], features["gtnc"][:, 12:15]], axis=1
        )
        placeholders["td_score_target"] = tf.stack(
            [features["gtnc"][:, 7], features["gtnc"][:, 11], features["gtnc"][:, 15]], axis=1
        )
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            per_sample_learning_rate = tf.constant(lr_base) * tf.compat.v1.train.piecewise_constant(
            ...
            with tf.control_dependencies(update_ops):
                ...
                for grad, x in gradients:
                    ...
                    if x.name in lr_adjusted_variables and grad is not None:
                        ...
                        if print_model:
                            trainlog(f"Adjusting gradient for {x.name} by {str(adj_factor)}")
            ...
            if print_model:
                ...
                for update_op in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS):
                    trainlog(f"Additional update op on train step: {update_op.name}")
                trainlog(f"Supporting japanese rules: {str(model.support_japanese_rules)}")

...
torch.set_printoptions(
    precision=7, sci_mode=False, linewidth=100000, edgeitems=1000, threshold=1000000
)
...
def get_outputs(gs, rules):
    with torch.no_grad():
        ...
        global_input_data = np.zeros(
            shape=[1] + model.global_input_shape, dtype=np.float32
        )
        pla = gs.board.pla
        ...
        td_value = (
            torch.nn.functional.softmax(td_value_logits[0, :], dim=0).cpu().numpy()
        )
        td_value2 = (
            torch.nn.functional.softmax(td_value_logits[1, :], dim=0).cpu().numpy()
        )
        td_value3 = (
            torch.nn.functional.softmax(td_value_logits[2, :], dim=0).cpu().numpy()
        )
        ...
        seki_probs = (
            torch.nn.functional.softmax(seki_logits[0:3, :, :], dim=0).cpu().numpy()
        )
        seki = seki_probs[1] - seki_probs[2]
        seki2 = torch.sigmoid(seki_logits[3, :, :]).cpu().numpy()
        scorebelief = (
            torch.nn.functional.softmax(scorebelief_logits, dim=0).cpu().numpy()
        )
    ...
    moves_and_probs = sorted(
        moves_and_probs0, key=lambda moveandprob: moveandprob[1], reverse=True
    )
...
def get_gfx_commands_for_heatmap(
    ...
    if hotcold:
        for loc, value in locs_and_values:
            if loc != Board.PASS_LOC:
                ...
                p = (value - interpoints[i][0]) / (
                    interpoints[i + 1][0] - interpoints[i][0]
                )
                ...
                b = "%02x" % int(b)
                gfx_commands.append(
                    "COLOR #%s%s%s %s" % (r, g, b, str_coord(loc, board))
                )
    ...
    locs_and_values = sorted(
        locs_and_values, key=lambda loc_and_value: loc_and_value[1]
    )
    locs_and_values_rev = sorted(
        locs_and_values, key=lambda loc_and_value: loc_and_value[1], reverse=True
    )
    texts = []
    texts_rev = []
    texts_value = []
...
while True:
    ...
    if command[0] == "boardsize":
        ...
        outputs = get_outputs(gs, rules)
        moves_and_logprobs = [
            (move, max(0.0, 4.9 + math.log10(prob)))
            for (move, prob) in outputs["moves_and_probs0"]
        ]
        gfx_commands = get_gfx_commands_for_heatmap(
            moves_and_logprobs,
            gs.board,
        ...
        gfx_commands = get_gfx_commands_for_heatmap(
            locs_and_values, gs.board, normalization_div, is_percent=False
        )
        ret = "\n".join(gfx_commands)
        ...
        gfx_commands = get_gfx_commands_for_heatmap(
            locs_and_values, gs.board, normalization_div=None, is_percent=False
        )
        ret = "\n".join(gfx_commands)

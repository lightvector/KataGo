...
def get_outputs(gs, rules):
    with torch.no_grad():
        ...
        features.fill_row_features(
            gs.board, pla, opp, gs.boards, gs.moves, move_idx, rules, bin_input_data, global_input_data, idx=0
        )
        bin_input_data = bin_input_data.reshape([1, pos_len, pos_len, -1])
...
def get_input_feature(gs, rules, feature_idx):
    ...
    move_idx = len(gs.moves)
    features.fill_row_features(
        board, pla, opp, gs.boards, gs.moves, move_idx, rules, bin_input_data, global_input_data, idx=0
    )
...
def get_pass_alive(board, rules):
    # pla = board.pla
    # opp = Board.get_opp(pla)
    area = [-1 for _ in range(board.arrsize)]
    nonPassAliveStones = False
    safeBigTerritories = True
    unsafeBigTerritories = False
    board.calculateArea(
        area, nonPassAliveStones, safeBigTerritories, unsafeBigTerritories, rules["multiStoneSuicideLegal"]
    )
...
def get_gfx_commands_for_heatmap(
    locs_and_values, board, normalization_div, is_percent, value_and_score_from=None, hotcold=False
):
    gfx_commands = []
    divisor = 1.0
    ...
    if hotcold:
        for loc, value in locs_and_values:
            if loc != Board.PASS_LOC:
                ...
                i = 0
                while i < len(interpoints) and not value <= interpoints[i][0]:
                    i += 1
                i -= 1
                ...
                g = "%02x" % int(g)
                b = "%02x" % int(b)
                gfx_commands.append(f"COLOR #{r}{g}{b} {str_coord(loc, board)}")
    ...
    if value_and_score_from is not None:
        ...
        texts_value.append(
            "wv %.2fc nr %.2f%% ws %.1f wl %.1f vt %.1f"
            % (
...
def add_input_feature_visualizations(layer_name, feature_idx, normalization_div):
    ...
    known_analyze_commands.append(f"gfx/{command_name}/{command_name}")
    input_feature_command_lookup[command_name] = (feature_idx, normalization_div)


for i in range(model.bin_input_shape[1]):
    add_input_feature_visualizations(f"input-{str(i)}", i, normalization_div=1)
...
while True:
    ...
    if command[0] == "boardsize":
        if int(command[1]) > features.pos_len:
            print(
                "Warning: Trying to set incompatible boardsize %s (!= %d)" % (command[1], board_size), file=sys.stderr
            )
            ret = None
        ...
        pla = Board.BLACK if command[1] in ["B", "b"] else Board.WHITE
        ...
        elif command[1] in ["whitekomi", "komi"]:
        ...
        outputs = get_outputs(gs, rules)
        gfx_commands = get_gfx_commands_for_heatmap(
            outputs["moves_and_probs0"], gs.board, normalization_div=None, is_percent=True, value_and_score_from=outputs
        )
        ret = "\n".join(gfx_commands)
    elif command[0] == "policy1":
        outputs = get_outputs(gs, rules)
        gfx_commands = get_gfx_commands_for_heatmap(
            outputs["moves_and_probs1"], gs.board, normalization_div=None, is_percent=True, value_and_score_from=outputs
        )
        ret = "\n".join(gfx_commands)
        ...
        gfx_commands = get_gfx_commands_for_heatmap(
            moves_and_logprobs, gs.board, normalization_div=6, is_percent=False, value_and_score_from=outputs
        )
        ret = "\n".join(gfx_commands)
    elif command[0] == "ownership":
        outputs = get_outputs(gs, rules)
        gfx_commands = get_gfx_commands_for_heatmap(
            outputs["ownership_by_loc"],
            gs.board,
            normalization_div=None,
            is_percent=True,
            value_and_score_from=None,
            hotcold=True,
        )
        ret = "\n".join(gfx_commands)
        ...
        outputs = get_outputs(gs, rules)
        gfx_commands = get_gfx_commands_for_heatmap(
            outputs["seki_by_loc"], gs.board, normalization_div=None, is_percent=True, value_and_score_from=None
        )
        ret = "\n".join(gfx_commands)
    elif command[0] == "seki2":
        outputs = get_outputs(gs, rules)
        gfx_commands = get_gfx_commands_for_heatmap(
            outputs["seki_by_loc2"], gs.board, normalization_div=None, is_percent=True, value_and_score_from=None
        )
        ret = "\n".join(gfx_commands)
        ...
        print(f"Warning: Ignoring unknown command - {line}", file=sys.stderr)
        ret = None

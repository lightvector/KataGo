...
def noisify(tensor):
    ...
    return tensor * (
        0.9 + 0.15 * torch.rand(tensor.shape, device=tensor.device)
    ) + scale * (2.0 * torch.rand(tensor.shape, device=tensor.device) - 1.0)
...
def expand_in_dim_for(name, scale):
    if f"module.{name}" in data["model"]:
        data["model"][f"module.{name}"] = double_linear_in_dim(
            f"module.{name}", data["model"][f"module.{name}"], scale
        )
...
def expand_out_dim_for(name, scale):
    if f"module.{name}" in data["model"]:
        data["model"][f"module.{name}"] = double_linear_out_dim(
            f"module.{name}", data["model"][f"module.{name}"], scale
        )
    elif name in data["model"]:
...
if any("intermediate_value_head" in key for key in data["model"].keys()):
    ...
    expand_in_dim_for(
        "intermediate_value_head.conv_ownership.weight", scale=math.sqrt(0.5)
    )
    expand_in_dim_for(
        "intermediate_value_head.conv_scoring.weight", scale=math.sqrt(0.5)
    )
    expand_in_dim_for("intermediate_value_head.linear_s2.weight", scale=math.sqrt(0.5))
    expand_in_dim_for(
        "intermediate_value_head.linear_smix.weight", scale=math.sqrt(0.5)
    )

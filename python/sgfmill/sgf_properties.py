...
def interpret_AP(s, context):
    ...
    if version is None:
        version = b""
    return (interpret_simpletext(application, context), interpret_simpletext(version, context))
...
def serialise_AP(value, context):
    ...
    return sgf_grammar.compose(serialise_simpletext(application, context), serialise_simpletext(version, context))
...
def serialise_ARLN_list(values, context):
    ...
    """
    return [serialise_point(p1, context) + b":" + serialise_point(p2, context) for p1, p2 in values]
...
def interpret_LB_list(values, context):
    ...
    for bb in values:
        ...
        result.append((interpret_point(point, context), interpret_simpletext(label, context)))
    return result
...
def serialise_LB_list(values, context):
    ...
    """
    return [serialise_point(point, context) + b":" + serialise_simpletext(text, context) for point, text in values]
...
def _make_property_type(type_name, allows_empty_list=False):
    return Property_type(
        globals()[f"interpret_{type_name}"],
        globals()[f"serialise_{type_name}"],
...
class Presenter(_Context):
    ...
    def __init__(self, size, encoding):
        try:
            ...
            raise ValueError("encoding names must be ascii") from e
        try:
    ...
    def _get_effective_property_type(self, identifier):
        try:
            ...
            if result is None:
                raise ValueError("unknown property") from e
            return result
    ...
    def serialise_as_type(self, property_type, value):
        ...
        if not property_type.uses_list:
            return [serialised]
        if serialised == []:

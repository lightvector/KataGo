...
def summarize_dir(dirpath):
    filenames = [
        filename for filename in os.listdir(dirpath) if filename.endswith(".npz")
    ]
    ...
    print(
        "Summarizing new dir with %d rows: %s" % (num_rows_this_dir, dirpath),
        flush=True,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-old-summary-file-to-assume-correct",
        required=False,
        help="Summary json file for directory contents",
    )
    parser.add_argument(
        "-new-summary-file",
        required=True,
    ...
    if old_summary_file_to_assume_correct is not None and os.path.exists(
        old_summary_file_to_assume_correct
    ):
    ...
    if (
        ...
        print(
            "Summary file written adding %d additional rows: %s"
            % (num_total_rows, new_summary_file),
            flush=True,
        )

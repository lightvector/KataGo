...
class Features:
    ...
    def iterLadders(self, board, f):
        ...
        for y in range(bsize):
            for x in range(bsize):
                ...
                if stone in [Board.BLACK, Board.WHITE]:
    ...
    def fill_row_features(
        self, board, pla, opp, boards, moves, move_idx, rules, bin_input_data, global_input_data, idx
    ):
        ...
        def addLadderFeature(loc, pos, workingMoves):
            assert board.board[loc] in [Board.BLACK, Board.WHITE]
        ...
        def addPrevLadderFeature(loc, pos, workingMoves):
            assert prevBoard.board[loc] in [Board.BLACK, Board.WHITE]
        ...
        def addPrevPrevLadderFeature(loc, pos, workingMoves):
            assert prevPrevBoard.board[loc] in [Board.BLACK, Board.WHITE]
        ...
        if rules["scoringRule"] == "SCORING_AREA" and rules["taxRule"] == "TAX_NONE":
            ...
            keepStones = False
            if rules["scoringRule"] == "SCORING_AREA" and rules["taxRule"] in [
                "TAX_SEKI",
                "TAX_ALL",
            ]:
                hasAreaFeature = True
                ...
                if rules["encorePhase"] >= 2:
                    ...
                    keepStones = False
            elif rules["scoringRule"] == "SCORING_TERRITORY" and rules["taxRule"] in [
                "TAX_SEKI",
                "TAX_ALL",
            ]:
                if rules["encorePhase"] >= 2:
            ...
            if hasAreaFeature:
                board.calculateNonDameTouchingArea(area, keepTerritories, keepStones, rules["multiStoneSuicideLegal"])
        ...
        if rules["koRule"] == "KO_SIMPLE":
            pass
        elif rules["koRule"] in ["KO_POSITIONAL", "KO_SPIGHT"]:

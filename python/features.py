...
class Features:
                    ...
                    if libs == 1 or libs == 2:
                        ...
                        if head in chainHeadsSolved:
                            ...
                            if libs == 1:
                                ...
                                workingMoves = (
                                    copy.searchIsLadderCapturedAttackerFirst2Libs(loc)
                                )
    ...
    def fill_row_features(
        ...
        def addPrevLadderFeature(loc, pos, workingMoves):
            assert (
                prevBoard.board[loc] == Board.BLACK
                or prevBoard.board[loc] == Board.WHITE
            )
        ...
        def addPrevPrevLadderFeature(loc, pos, workingMoves):
            assert (
                prevPrevBoard.board[loc] == Board.BLACK
                or prevPrevBoard.board[loc] == Board.WHITE
            )
        ...
        if rules["scoringRule"] == "SCORING_AREA" and rules["taxRule"] == "TAX_NONE":
            ...
            keepStones = False
            if rules["scoringRule"] == "SCORING_AREA" and (
                rules["taxRule"] == "TAX_SEKI" or rules["taxRule"] == "TAX_ALL"
            ):
                hasAreaFeature = True
                keepTerritories = False
                keepStones = True
                ...
                and rules["taxRule"] == "TAX_NONE"
            ):
                if rules["encorePhase"] >= 2:
                    hasAreaFeature = True
                    keepTerritories = True
                    keepStones = False
            elif rules["scoringRule"] == "SCORING_TERRITORY" and (
                rules["taxRule"] == "TAX_SEKI" or rules["taxRule"] == "TAX_ALL"
            ):
                if rules["encorePhase"] >= 2:
                    hasAreaFeature = True
            ...
            if hasAreaFeature:
                board.calculateNonDameTouchingArea(
                    area, keepTerritories, keepStones, rules["multiStoneSuicideLegal"]
                )
        ...
        if (
            "hasButton" in rules
            and rules["hasButton"]
            and Board.PASS_LOC not in [move[1] for move in moves]
        ):

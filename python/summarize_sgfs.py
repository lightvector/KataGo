...
class GameResultSummary:
    ...
    def __init__(
        ...
        self._elo_prior_games = (
            elo_prior_games  # number of games for bayesian prior around Elo 0
        )
    ...
    def print_elos(self):
        ...
        real_players = [
            player for player in elo_info.players if player != elo.P1_ADVANTAGE_NAME
        ]
        ...
        print(
            f"Used a prior of {self._elo_prior_games} games worth that each player is"
            " near Elo 0."
        )
        if self._should_warn_handicap_komi:
            print(
                "WARNING: There are handicap games or games with komi < 5.5 or komi >"
                " 7.5, these games may not be fair?"
            )
    ...
    def _add_files(self, input_file_or_dir, recursive):
        ...
        if not os.path.exists(input_file_or_dir):
            raise Exception(
                f"There is no file or directory with name: {input_file_or_dir}"
            )
        ...
        new_sgfs_files = set(
            [file for file in files if file.split(".")[-1].lower() == "sgfs"]
        )
        new_sgf_files = set(
            [file for file in files if file.split(".")[-1].lower() == "sgf"]
        )
        ...
        self._add_new_games_to_result_dict(
            new_sgf_files, new_sgfs_files, input_file_or_dir
        )

        print(
            f"Added {len(new_sgfs_files)} new sgfs files and {len(new_sgf_files)} new"
            f" sgf files from {input_file_or_dir}"
        )
    ...
    def _add_a_single_sgf_string(self, sgf_string, debug_source=None):
        ...
        try:
            ...
            print(
                "\033[91m"
                + f"A sgf string is damaged in {debug_source}, and its record has been"
                " skipped!"
                + "\x1b[0m"
            )
            return
        ...
        if (
            (game.get_handicap() is not None)
            or game.get_komi() < 5.5
            or game.get_komi() > 7.5
        ):
    ...
    def _estimate_elo(self) -> elo.EloInfo:
        ...
        data.extend(
            elo.make_center_elos_prior(list(pla_names), 0)
        )  # Add this in case user put elo_prior_games = 0
        if self._estimate_first_player_advantage:
            data.extend(
                elo.make_single_player_prior(
                    elo.P1_ADVANTAGE_NAME, (1.0 + self._elo_prior_games) * 2.0, 0
                )
            )
    ...
    def _print_matrix(self, pla_names, results_matrix):
        ...
        per_name_space = (
            1 if len(pla_names) == 0 else max(len(name) for name in pla_names)
        )
        per_name_space += 1
        if per_name_space > per_elt_space:
            per_elt_space += 1

        row_format = f"{{:>{per_name_space}}}" + f"{{:>{per_elt_space}}}" * len(
            results_matrix
        )
        ...
        for pla1 in pla_names:
            ...
            for pla2 in pla_names:
                if pla1 == pla2:
                    ...
                    pla1_pla2 = (
                        self.results[(pla1, pla2)]
                        if (pla1, pla2) in self.results
                        else Record()
                    )
                    pla2_pla1 = (
                        self.results[(pla2, pla1)]
                        if (pla2, pla1) in self.results
                        else Record()
                    )
                    total += (
                        pla1_pla2.win
            ...
            for pla2 in pla_names:
                if pla1 == pla2:
                    ...
                    win = (
                        pla1_pla2.win
                        + pla2_pla1.lost
                        + 0.5 * (pla1_pla2.draw + pla2_pla1.draw)
                    )
                    total = (
                        pla1_pla2.win
                        + pla2_pla1.win
...
if __name__ == "__main__":
  ...
  """
    ...
    parser.add_argument(
        "-elo-prior-games",
        help=(
            "Prior for Bayes Elo calculation, using input as the prior number of games"

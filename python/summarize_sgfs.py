...
class GameResultSummary:
    ...
    def print_game_results(self):
        ...
        pla_names = set(itertools.chain(*iter(self.results.keys())))
    ...
    def print_elos(self):
        ...
        if self._should_warn_handicap_komi:
            print(
                "WARNING: There are handicap games or games with komi < 5.5 or komi > 7.5, these games may not be fair?"
            )
    ...
    def _add_files(self, input_file_or_dir, recursive):
        ...
        if not os.path.exists(input_file_or_dir):
            raise ValueError(f"There is no file or directory with name: {input_file_or_dir}")
        ...
        new_sgfs_files = {file for file in files if file.split(".")[-1].lower() == "sgfs"}
        new_sgf_files = {file for file in files if file.split(".")[-1].lower() == "sgf"}
        ...
        print(
            f"Added {len(new_sgfs_files)} new sgfs files and {len(new_sgf_files)} new sgf files from"
            f" {input_file_or_dir}"
        )

    def _add_new_games_to_result_dict(self, new_sgf_files, new_sgfs_files, source):
        for idx, sgfs in enumerate(new_sgfs_files, start=1):
            self._add_one_sgfs_file_to_result(sgfs)
            if idx % 10 == 0:
                print(f"Added {idx}/{len(new_sgfs_files)} sgfs files for {source}")

        for idx, sgf_file in enumerate(new_sgf_files, start=1):
            self._add_one_sgf_file_to_result(sgf_file)
            if idx % 10 == 0:
    ...
    def _add_one_sgfs_file_to_result(self, sgfs_file_name):
        ...
        if not os.path.exists(sgfs_file_name):
            raise ValueError(f"There is no SGFs file named: {sgfs_file_name}")

        with open(sgfs_file_name, "rb") as f:
            sgfs_strings = f.readlines()

        for sgf_string in sgfs_strings:
            self._add_a_single_sgf_string(sgf_string, sgfs_file_name)

    def _add_one_sgf_file_to_result(self, sgf_file_name):
        """Add a single sgf file."""
        if not os.path.exists(sgf_file_name):
            raise ValueError(f"There is no SGF file named: {sgf_file_name}")
    ...
    def _add_a_single_sgf_string(self, sgf_string, debug_source=None):
        ...
        try:
            ...
            print(
                "\033[91m" + f"A sgf string is damaged in {debug_source}, and its record has been skipped!" + "\x1b[0m"
            )
            return
    ...
    def _print_result_matrix(self, pla_names):
        ...
        for pla1 in pla_names:
            ...
            for pla2 in pla_names:
                ...
                total += (
                    pla1_pla2.win + pla2_pla1.win + pla1_pla2.lost + pla2_pla1.lost + pla1_pla2.draw + pla2_pla1.draw
                )

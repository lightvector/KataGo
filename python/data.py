...
def load_sgf_moves_exn(path):
    ...
    if ab or aw:
        ...
        if color is not None:
            raise ValueError("Found both setup stones and normal moves in root node")
    ...
    while node:
        ...
        if node.has_setup_stones():
            raise ValueError("Found setup stones after the root node")

        color, raw = node.get_raw_move()
        if color is None:
            raise ValueError("Found node without move color")
        ...
        if color == "b":
            ...
            raise ValueError(f"Invalid move color: {color}")
        ...
        if pla == prev_pla and (prev_pla != Board.BLACK or seen_white_moves):
            raise ValueError("Multiple moves in a row by same player")
        moves.append((pla, loc))

        prev_pla = pla
        if prev_pla == Board.WHITE:
    ...
    if rulesstr is not None:
        if rulesstr.lower() in ["japanese", "jp"]:
            rules = {
                "koRule": "KO_SIMPLE",
...
def parse_rules_str(rulesstr):
    ...
    if rulesstr.startswith("SIMPLE"):
        ...
        raise ValueError(f"Could not parse rules: {origrulesstr}")

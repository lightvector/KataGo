...
## Current results
Currently, the best neural nets I've been training from this sandbox have about 3.28 million parameters, arranged as a resnet with about 6 blocks with a main trunk of 192 channels, and a policy head, trained for about 52 million samples. On GoGoD games from 1995 to mid 2016, which is about 50000 games with about 10 million training samples, the validation cross-entropy loss achieved with this many parameters is about 1.63 nats, corresponding to a perplexity of exp(1.63) = 5.10. So the neural net has achieved close to the same entropy on average as if on every move it could pin down the next move uniformly to 5 possible moves. The top-1 prediction accuracy is about 51.9%.

I haven't tried running on the KGS dataset yet that most papers out there tend to test on. From the few papers that test on both GoGoD and KGS it seems very roughly that KGS is a bit easier to predict, with accuracies about 4% lower for the pro games than for KGS amateur games, so probably these results correspond to roughly a 56% top-1 accuracy on KGS, which I'll get around to testing directly eventually. I'm also only running on a smaller date range from GoGoD than most papers report, so the numbers in the table below are not 100% comparable.

I also tried testing a recent neural net against GnuGo (not actually the latest and best as I did this test weeks ago, but fairly recent). The policy prediction was converted to a move choice by sorting the predictions by probability, then selecting from a biased distribution that early in the game favored randomization and late in the game would almost always choose the top choice. Out of 250 games, it lost only 3. So things are developing pretty well.

For a comparison with other neural nets, see this table for a summary of results:

...
- [KataGo](#katago)
  ...
  - [Setting Up and Running KataGo](#setting-up-and-running-katago)
    ...
    - [MacOS](#macos)
    - [OpenCL vs CUDA vs TensorRT vs ROCm vs ONNX vs Eigen](#opencl-vs-cuda-vs-tensorrt-vs-rocm-vs-onnx-vs-eigen)
...
The quick summary is:
  * **To easily get something working, try OpenCL if you have any good or decent GPU.** Note that it is noticeably slower than the NVIDIA-specific backends, especially for transformer models.
  ...
  * The ONNX Runtime backend is mainly useful for hardware with no native KataGo backend, most notably Intel GPUs and NPUs (via its OpenVINO execution provider) and any DirectX 12 GPU on Windows (via DirectML).
...
More in detail:
  ...
  * ONNX runs the neural net using [ONNX Runtime](https://onnxruntime.ai/), which supports many kinds of hardware through its "execution providers". This is mainly useful for hardware that has no native KataGo backend, such as Intel GPUs and NPUs (via the OpenVINO provider) or any DirectX 12 GPU on Windows (via the DirectML provider). If you have an NVIDIA GPU, use KataGo's native CUDA or TensorRT backends instead - they will almost certainly be faster. See [Compiling.md](Compiling.md#onnx-runtime-backend-optional) for the status of the various execution providers and what each one requires.

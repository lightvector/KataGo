...
| Current Sandbox | ResNet 5 Blocks | 3,597,792 | 55.9% | 51.5% | 65M (partial train) |
| Current Sandbox | ResNet 5 Blocks | 3,597,792 | 56.8% | 52.5% | 205-209M | 99.2% (496/500) |
| Current Sandbox | ResNet 12 Blocks | 8,250,720 | 56.8% | 52.7% | 65M (partial train) |
| Current Sandbox | ResNet 12 Blocks | 8,250,720 | 58.2% | 54.2% | 325-333M | 100% (500/500) | 90.0% (36/40)

Unfortunately, not all of the results in this table are precisely comparable. In the above papers, different authors often made different choices about many minor details or didn't say what choices they made for these details. For example in GoGoD there is a choice of the what date range of pro games to train or test on, as well as the GoGoD version itself (e.g. Winter 2015, or Summer 2016, etc) which affects what game records are available. For KGS games there is also the choice of date range, and also a choice of what ranks of players to allow, and whether to include or exclude handicap games. Overall, I think most of these choices shouldn't affect the results too much, but I would guess could affect accuracy numbers by a reasonable fraction of a percent.

There is also one interesting choice that some of these papers differ on, which is whether to use cross entropy or L2 loss when training the neural net. Although I haven't performed detailed experiments, in at least two of the training runs for my own neural nets to produce results for the above table, I found that using L2 loss increased the test set Top1 accuracy by about 0.2% while decreasing the Top4 accuracy by about 0.2% and the average log likelihood (i.e. the cross entropy loss itself) by about 0.005 nats. Directionally, this is in line with what one might expect from theoretical priors. L2 loss "cares" relatively more about the total probability mass of correct classifications in cases with a few choices, while cross-entropy "cares" more about avoiding putting too small of probability on surprising moves. So it's not surprising that L2 does slightly better at Top1 while cross entropy does slightly better in the tail. For a nice discussion of loss functions, see also: https://arxiv.org/pdf/1702.05659.pdf

For this sandbox, the neural nets I tested had up to 12 residual blocks with a main trunk of up to 192 channels. Not all of the enhancements discussed further below were actually used in the most recent neural nets, but many were, the most important of which were the global pooling and parametric relus, and secondarily the dilated convolutions and the position-dependent learning rates inspired by "redundant" parametrizations. I also trained neural nets separately on each of KGS and GoGoD in each case. And to generate the numbers in the above table, I tried to match the choices from Cazenave (2017):
   ...
   * KGS - I used the "one player is at least 7 dan or both players at least 6 dan" data set from https://u-go.net/gamerecords/ from 2000 to 2014 as the training set, and 2015 as the testing set. I excluded all handicap games and also filtered only to moves made by players 6d or stronger (in rare cases, a 5d or weaker player might play a 7d or stronger player in a non-handicap game).

Also, both the 12 block KGS and GoGoD neural nets initially used cross entropy loss, but partway through when I realized the L2/cross-entropy difference between the various papers, I switched to L2 loss for a better comparison with Cazenave's results which also use L2 loss. For the 5 block nets though, I used only cross entropy. For each loss function, I briefly by hand tried a few different learning rate factors to see roughly what worked best, since a little bit of learning rate tuning is needed to deal with the fact that L2 and cross entropy are a small constant factor different in typical magnitude and gradient.
...
Like chain pooling, holding the number of parameters constant I did not find it to noticeably improve the cross-entropy loss. However when replacing 64/192 of the channels in this way, the loss also did not get worse either, unlike how it would get worse if one were to simply delete those channels. Combined with the above pictures, it suggests the neural net is definitely putting the dilations to good use to learn longer-distance relationships at the cost of being a little worse at some local shapes or tactics. And unlike chain pooling, dilated convolutions appear to be quite cheap, barely at all increasing the computational cost of the neural net.

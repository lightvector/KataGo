...
KataGo has five backends, OpenCL (GPU), CUDA (GPU), TensorRT (GPU), Eigen (CPU), and ONNX (CPU/CoreML).
...
The quick summary is:
  ...
  * Use ONNX if you want to load raw `.onnx` model files or use CoreML on Apple Silicon.
...
More in detail:
  ...
  * ONNX is a backend that uses [ONNX Runtime](https://onnxruntime.ai/) for inference. It can load both standard `.bin.gz` model files and raw `.onnx` model files directly. It supports CPU inference out of the box, and on macOS it can use the CoreML execution provider for hardware-accelerated inference on Apple Silicon. Requires building ONNX Runtime from source as a prerequisite. See [Compiling KataGo](Compiling.md) for details.
